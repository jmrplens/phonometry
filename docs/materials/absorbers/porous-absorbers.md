...
Five parameters is four more than a caller usually has, so the specimens this
guide computes with are published as `PUBLISHED_POROUS`, keyed by the table
they were read from and the row they are: `'allard-2009-table-6-1/domisol_coffrage'`
is the glass wool of Allard & Atalla Table 6.1 (printed p. 124), whose
characteristic lengths are printed in the prose of their Sect. 6.5.4 on the
facing folio, and `'allard-2009-table-11-2/soft_fibrous'` is the soft fibrous
layer of their Table 11.2 (printed p. 254). The key names the table because
the book prints one foam in four chapters and one glass wool in two, sometimes
with a column the other page leaves out, and picking silently between two
published parameter sets is the thing a catalogue exists not to do.

Every quantity of a row is optional, because the pages print different
columns. `.why_missing(field)` answers in the page's own terms: Table 11.5
prints the word `model` where a screen's tortuosity would go, because the
quantity is frequency dependent there and no single number stands for it, and
that is a different answer from a cell the book left empty. `.printed(field)`
is the same reading for a caller who needs the number and would rather be
refused than handed a `None`.

These are parameter sets from worked examples and not a material database: a
real specimen is characterised to ISO 9053 and ISO 10534-2, and every model
here keeps its parameters as explicit arguments.
...
Each says what it is worth. Hopkins' two equations check each other: the ends of
the bulk-density range his resistivity fit was made over, put through his
porosity relation at the fibre density he states, give back the porosity range he
...
soft = materials.PUBLISHED_POROUS["allard-2009-table-11-2/soft_fibrous"]
print(soft.source)
