...
velocidades. `PUBLISHED_SOLIDS` tiene setenta y ocho filas leídas de tres páginas
...
Tres tablas imprimen un acero, con la misma densidad, y no son el mismo acero:
...
Mechel imprime módulo y ningún coeficiente de Poisson, así que sus filas dan la
velocidad de barra y ahí se paran. La de placa y la de sólido extenso pedirían
un coeficiente que él no imprime, y un 0,3 reproduciría su tabla estupendamente
y sería un número que él no dio.
...
Lo que cruza entre los tres libros sin ninguna suposición es `h f_c`, el
...
otra columna: para el acero, los dos imprimen 12,3 m Hz.
