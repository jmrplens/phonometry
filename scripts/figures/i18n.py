...
_ES_EXACT = {
    ...
    # The London sites of the soundscape guide are place names: the same in Spanish.
    "Camden Town": "Camden Town",
    "Euston Tap": "Euston Tap",
    "Marchmont Garden": "Marchmont Garden",
    "Pancras Lock": "Pancras Lock",
    "Regent's Park Fields": "Regent's Park Fields",
    "Regent's Park Japan": "Regent's Park Japan",
    "Russell Square": "Russell Square",
    "St Paul's Cross": "St Paul's Cross",
    "St Paul's Row": "St Paul's Row",
    "Tate Modern": "Tate Modern",
    "Torrington Square": "Torrington Square",
    # The metric symbols of ISO/TS 12913-3 Table D.1, printed the same in any
    # language: they are the standard's symbols, not English words.
    r"$L_{\mathrm{Aeq},T}$": r"$L_{\mathrm{Aeq},T}$",
    r"$L_{\mathrm{Ceq},T}$": r"$L_{\mathrm{Ceq},T}$",
    r"$N_\mathrm{average}$": r"$N_\mathrm{average}$",
    r"$N_\mathrm{rmc}$": r"$N_\mathrm{rmc}$",
    r"Rank of the site pleasantness $P$": r"Rango de la agradabilidad $P$ del lugar",
    r"Rank of the site $L_\mathrm{Aeq}$": r"Rango del $L_\mathrm{Aeq}$ del lugar",
    "An array is not one device repeated": "Un array no es un dispositivo repetido",
    "Six semiellipses, one width, four depths": "Seis semielipses, una anchura, cuatro profundidades",
    "1 semicylinder": "1 semicilindro",
    "2 semicylinders": "2 semicilindros",
    "6 semicylinders": "6 semicilindros",
    "12 semicylinders": "12 semicilindros",
    ...
    # people_guide_values and people_trains_per_hour (DIN 4150-2): Table 1 by
    # area, and Figure D.1 with the two readings Annex D quotes.
    ...
    "$n$ = 0.5": "$n$ = 0,5",
    "$n$ = 1": "$n$ = 1",
    "measured with group Gd running": "medido con el grupo Gd en marcha",
    "$\\alpha$ = 0.005 1/m from $D$ = 0.01 and $\\lambda$ = 12.5 m,\nthe same damping on every curve": "$\\alpha$ = 0,005 1/m con $D$ = 0,01 y $\\lambda$ = 12,5 m,\nel mismo amortiguamiento en todas las curvas",
    "Distance from the source $R$ [m]": "Distancia a la fuente $R$ [m]",
    "Vertical velocity $v_z$ [mm/s]": "Velocidad vertical $v_z$ [mm/s]",
    "A hall of similar machines, measured with $N_B$ of them (DIN 4150-1 Figure 3)": "Una nave de máquinas iguales, medida con $N_B$ de ellas (DIN 4150-1, figura 3)",
    "$N_B$ = 3": "$N_B$ = 3",
    "$N_B$ = 5": "$N_B$ = 5",
    "$N_B$ = 10": "$N_B$ = 10",
    "$N_B$ = 30": "$N_B$ = 30",
    "$N_B$ = 60": "$N_B$ = 60",
    "$N_B$ = 100": "$N_B$ = 100",
    "read off the printed nomogram\nat a five-hundredth, the stroke's width": "leído del nomograma impreso\na una quingentésima, el grosor del trazo",
    "Machines running $N$": "Máquinas en marcha $N$",
    "Correction $\\chi$ of Formula (7)": "Corrección $\\chi$ de la fórmula (7)",
    ...
    "the 16 subjects": "los 16 sujetos",
    r"$\pm U_{95}$ of the mean": r"$\pm U_{95}$ de la media",
    ...
    "shaded: above $f_\\mathrm{UL}$, not evaluated": "$k_{2,1}$ = 1 MN/m, $\\eta$ = 0,05, placa de reparto de fuerza de 2 kg\n"
    ...
    "Octave bank, decimated: class 1 on §5.16": "Banco de octava, diezmado: clase 1 en §5.16",
    ...
    "Reference level [dB]": "Nivel de referencia [dB]",
    "Beyer DT 48 (IEC 60303 coupler)": "Beyer DT 48 (acoplador CEI 60303)",
    "Telephonics TDH 39 (same coupler)": "Telephonics TDH 39 (mismo acoplador)",
    "any other supra-aural (IEC 60318 ear)": "cualquier otro supraaural (oído CEI 60318)",
    "TDH 39, coupler": "TDH 39, acoplador",
    "ISO 389-7, free field": "ISO 389-7, campo libre",
    "the two zeros are not the same": "los dos ceros no son el mismo",
    ...
    "echo / incident = 0.939\n$(Z_2 - Z_1)/(Z_2 + Z_1)$ = 0.939": "eco / incidente = 0,939\n$(Z_2 - Z_1)/(Z_2 + Z_1)$ = 0,939",
