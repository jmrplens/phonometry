...
a value the page prints in another unit, degrees Fahrenheit, psi or sabins, is
converted, and the mark names the figure and the unit it prints; and a cell
the page leaves blank because it prints the value once for a block of rows is
carried, and the mark names the row that prints it. A value the source itself
calls an estimate is marked as one.
