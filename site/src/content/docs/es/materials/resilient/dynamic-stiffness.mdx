...
del argumento, `airflow_resistivity_kpa_s_m2`, el único de esta página que no
está en SI puro, y solo lo acepta por nombre, para que la unidad quede escrita
en la línea que lo pasa. Una lana mineral medida en 50 000 Pa·s/m² entra
...
**Una capa que solo da $s'_\mathrm{t}$.** Un informe de ensayo puede dar la
rigidez aparente de su probeta y nada más, y $s'_\mathrm{t}$ no es la $s'$ que
toma la Fórmula 2: en una capa permeable al aire el gas encerrado suele ser la
mayor parte de $s'$. Una `ResilientLayer` construida a partir de ese informe la
guarda en `apparent_dynamic_stiffness_n_m3`, y su `natural_frequency` se niega
a seguir hasta que recibe la resistividad al flujo lateral, en Pa·s/m² como la
da la ISO 9053, y, por debajo de 100 kPa·s/m², la rigidez del gas encerrado de
la Fórmula 7. Con ellas toma la rama del apartado 8.2 que elige esa
resistividad:
...
try:
    ...
    print(refusal)   # s't no es s': pasa airflow_resistivity_pa_s_m2, gas_stiffness_n_m3
