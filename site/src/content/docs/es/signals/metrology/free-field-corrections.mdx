...
<ThemeImage src="https://raw.githubusercontent.com/jmrplens/phonometry/main/.github/images/free_field_correction.svg" alt="Dos paneles frente a la frecuencia en las nueve octavas exactas de 63 Hz a 16 kHz. Izquierda: la corrección de campo libre de un sonómetro sintético con un calibrador multifrecuencia por la Fórmula (D.7), cerca de 0 dB hasta 1 kHz y subiendo a 0,6 dB a 4 kHz, 1,7 dB a 8 kHz y 5 dB a 16 kHz, una banda estrecha a su alrededor para el intervalo de tres micrófonos, y la corrección de campo libre del micrófono de referencia en trazos debajo. Derecha: la corrección del mismo sonómetro con un actuador electrostático por la Fórmula (F.13), nula a 1 kHz por construcción y subiendo a 7,4 dB a 16 kHz, con la sensibilidad normalizada del canal de referencia en trazos" width="100%" />
...
<ThemeImage src="https://raw.githubusercontent.com/jmrplens/phonometry/main/.github/images/diagram_free_field_corrections_setup.svg" alt="Las lecturas de IEC 62585 en tres columnas, cada una por sustitución. A la izquierda, un calibrador acústico (Anexo D): 1, el sonómetro en campo libre lee L_ind1; 2, el micrófono de referencia de tipo LS2P en su lugar lee L_ind2; 3, el calibrador sobre el sonómetro lee L_ind3; 4, el calibrador sobre la referencia lee L_ind4; la Fórmula (D.7) da (L_ind1 - L_ind3) - (L_ind2 - L_ind4) + C_FF,RM. En el centro, un acoplador de comparación (Anexo E): las mismas dos lecturas en campo libre, y después la referencia y el sonómetro enfrentados en el acoplador, leídos a la vez como L_ind3a y L_ind3b; la Fórmula (E.6), con las etiquetas de la Figura E.1, da (L_ind1 - L_ind3b) - (L_ind2 - L_ind3a) + C_FF,RM. A la derecha, un actuador electrostático (Anexo F): las dos lecturas en campo libre en función de la frecuencia, y después el actuador sobre la membrana del sonómetro lee L_ind3(f); por la Fórmula (F.13) cada término se refiere a f_0, donde la corrección es nula. El pie dice que cada par se lee por turnos en el mismo lugar (Anexo G) y que C_FF,RM sale de la IEC/TS 61094-7" width="100%" />
...
| Nivel de la fuente | De 70 dB a 125 dB en un calibrador o un acoplador, preferiblemente el nivel de referencia a 1 kHz; de 80 dB a 105 dB en la pantalla con un actuador | 12, 13, 14 |
...
las etiquetas de la figura, la (E.6) impresa queda a dos veces la diferencia
entre las dos respuestas de presión de la corrección, lo que llega a
decibelios por encima de unos pocos kilohercios; el defecto está en el
...
libertad. Al 95 % el factor de Student para 29,98 es 2,04; 2,11 es el de unos
17. La incertidumbre expandida que dan los números de la página es
0,120 dB, que se imprime 0,12 en cualquier caso, pero el dígito de guarda de
...
**IEC 62585:2012, los cálculos de todos los métodos:** el valor de ajuste del
Anexo A, con la corrección de presión a campo libre y los niveles $L_1$ a
$L_4$ de la Figura A.1; las correcciones de campo libre para un calibrador
acústico multifrecuencia (Fórmula (D.7)), un acoplador de comparación (Fórmula
(E.6), con las lecturas nombradas por lo que miden) y un actuador
electrostático (Fórmula (F.13), normalizada a la frecuencia de comprobación de
la calibración), promediadas entre las determinaciones y conservando su
intervalo; el presupuesto de incertidumbre del Anexo I con las quince
componentes de la Tabla I.1, la componente de presión estática del apartado 6
y el factor de cobertura de Welch-Satterthwaite; los máximos de los apartados
9 a 14 y el requisito sobre el intervalo de los apartados 12 a 14 como
dictamen y como ficha; y las frecuencias exactas del Anexo H. Las Tablas I.2,
I.3 y H.1 se reproducen.
</ScopeClaim>
...
de la IEC/TS 61094-7 o de su calibración y aquí es una entrada. La
comparación de las correcciones declaradas con los límites de aceptación de la
