...
Una forma de onda vuelve con el tipo con el que entró. `weighting_filter`,
`linkwitz_riley`, `parametric_eq` y `fractional_delay` devuelven un `Signal`
para un `Signal`, y los objetos de resultado llevan el suyo del mismo modo:
`envelope(...).signal`, `time_synchronous_average(...).period_waveform`,
`resample_signal(...).signal` a su nueva frecuencia de muestreo, los
`aligned` y `reference` de `align_impulse_responses`, y la `pressure` de
`underwater.pile_strike_metrics`. Un `Signal` devuelto lleva
`calibration_factor=1.0` cuando la entrada estaba calibrada, lo que dice que
la conversión a pascales ya está hecha, así que pasarlo a otra función no
aplica el factor dos veces. Un array desnudo entra y sale desnudo.
...
vez. `read_blocks()` entrega bloques `Signal`, cada uno lo que `read()`
devolvería para el mismo tramo: el mismo escalado y el mismo convenio de
canales (en la instalación base, con posicionamiento y descodificación
propios para el WAV lineal, RF64 incluido), y la misma frecuencia de
muestreo, calibración, etiquetas de canal y procedencia a bordo. La
calibración sigue la regla de `read()`, así que un factor pasado aquí o
encontrado en el sidecar hace que cada bloque presente sus muestras en
pascales a los filtros y a las funciones de nivel, y no se aplica ningún
factor a mano. Los filtros con estado de la guía de
[proceso por bloques](/phonometry/es/signals/filters/block-processing/)
consumen el flujo sin cambios, y la aritmética de abajo toma `np.asarray` de
lo que devuelven, como haría con cualquier `Signal`:
...
# por bloques 67.2940 dB, archivo entero 67.2940 dB
```
...
sabe), 32 bits, float de 32/64. Promociona a RF64 automáticamente
...
lectores de aquí entienden. El sidecar se copia byte a byte, y una línea
