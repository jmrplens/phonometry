...
for temp, rh in [(20.0, 50.0), (20.0, 10.0), (0.0, 70.0), (30.0, 80.0)]:
    ax.loglog(freqs, environment.air_attenuation(freqs, temperature_c=temp, relative_humidity_percent=rh) * 1000.0,
...
cell = environment.air_attenuation(1000.0, temperature_c=10.0, relative_humidity_percent=70.0,
                                exact_midband=True) * 1000.0
...
freqs = np.geomspace(50.0, 10000.0, 400)
fig, ax = plt.subplots()
ax.semilogx(freqs, environment.air_attenuation(freqs, temperature_c=20.0, relative_humidity_percent=50.0) * 1000.0)
ax.set_xlabel("Frequency [Hz]")
ax.set_ylabel("Attenuation coefficient alpha [dB/km]")
plt.show()
```
...
result = environment.outdoor_propagation_attenuation(
    200.0, source_height=4.0, receiver_height=2.0, frequencies=freqs,
    ground_source=1.0, ground_middle=1.0, ground_receiver=1.0,
