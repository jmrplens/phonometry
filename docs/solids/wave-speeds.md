...
working. `check_solid_agreement.py` puts them side by side and fails on
density, which is the one column a table cannot get very wrong without
describing a different material: across these six tables a named material's
density agrees to within 2,6 per cent, while the moduli disagree by nineteen
per cent on aluminium and thirty on lead without anybody being wrong. Its first
full run found tin, where Cremer and Mechel print 4,4 GPa and Bies 54 while the
three densities agree to 0,3 per cent. Each book is consistent with itself, so
no page contradicts itself and this is not an errata; the accepted modulus of
tin is near Bies' figure rather than the one the other two share, which is why
the catalogue keeps all six books and nothing in it votes by majority.
