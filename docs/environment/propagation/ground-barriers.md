...
### A ground from a catalogue

A row of `environment.PUBLISHED_GROUND`, or of [a catalogue of your own](../../io/material-catalogues.md),
gives the impedance itself: `GroundSurface.medium` is the porous half-space
of its effective flow resistivity, by the Delany and Bazley or the Miki
model, which `ground_effect` takes as `impedance`, `barrier_insertion_loss`
as `ground_impedance` and `atmospheric_parabolic_equation` as `impedance`.
The resistivity is read through `printed()`, so a row that prints a range
is refused rather than averaged, and, because the fitted value is
model-bound, a row the page marks as fitted with one model goes into that
model only. Harmonoise class D, $\sigma$ = 200 kPa·s/m², gives the Miki
curve of the case above:

```python
octaves = [125.0, 250.0, 500.0, 1000.0]
...
print(over_pasture.excess_attenuation.round(2))   # [  3.84  -3.18 -13.19  -2.96]

field = environment.PUBLISHED_GROUND["cox-2017-table-6-7/sports_field_delany_bazley"]
try:
    field.medium(octaves, model="miki")
except ValueError as error:
    print(error)
# 'Sports field' is the row the page marks “Fitted using Delany and Bazley
...
# not of 'miki'; ask for model='delany_bazley', or take
# row.printed('flow_resistivity_pa_s_m2') into another model yourself.
```
