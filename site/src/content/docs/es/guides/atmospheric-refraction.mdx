...
transforma de vuelta y aplica la pantalla de fase de refracción
`exp(i Δr (k(z) − ka))` (Ec. H.58). La fuente es un **arranque gaussiano** con
...
complejo normalizado en el convenio `e^{-iωt}` con `Im(Z) < 0` para un suelo
pasivo), como un `PorousMediumResult`, o a partir de una `flow_resistivity`
efectiva mediante los [modelos porosos](/phonometry/es/guides/porous-absorbers/)
...
- **Límite homogéneo → efecto suelo esférico.** Con gradiente nulo el campo
  ...
  exacto de Weyl-Van der Pol a cualquier distancia, con unos **0,1 dB** sobre
  pasto, y el refuerzo coherente de **+6 dB** de los dos rayos sobre un suelo
  rígido.
