...
class FieldNameInProseError(Exception):
    """A text the page shows a reader names a field by its identifier.

    A note or a derived text is written for someone reading a row, and
    ``plate_longitudinal_speed_m_s`` is a name for code: the text has to say
    "the plate speed", in English and in Spanish.
    """


def refuse_field_names(document: dict[str, Any]) -> None:
    """Refuse a row whose words, in either language, carry a column's field name.

    :param document: What :func:`render` writes, after :func:`in_spanish`.
    :raises FieldNameInProseError: Naming each row and field name found.
    """
    names = {
        column["field"]
        for table in document.values()
        for column in table.get("columns", [])
        if "_" in column.get("field", "")
    }
    pattern = re.compile(
        r"(?<![A-Za-z0-9_])("
    ...
    for table_name, table in document.items():
        for row in table["rows"]:
            ...
            found.extend(
                ...
                for text in texts
                if isinstance(text, str) and (hit := pattern.search(text))
            )
    if found:
        msg = "texts shown to readers name fields by their identifiers:\n  " + (
            "\n  ".join(sorted(set(found)))
        )
        raise FieldNameInProseError(msg)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        fresh = render()
    except FieldNameInProseError as error:
