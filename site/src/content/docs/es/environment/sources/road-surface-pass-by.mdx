...
references:
  ...
  - type: standard
    ...
    note: "Los límites de textura y de absorción que tiene que cumplir el pavimento de referencia general de 10.1."
  - type: standard
    organization: "International Organization for Standardization"
    year: 1997
...
<ThemeImage
  ...
  alt="Nivel máximo ponderado A frente a la velocidad del vehículo en un eje logarítmico de unos 55 a 125 km/h en un emplazamiento de velocidad media: una nube de 120 turismos, 40 pesados de dos ejes y 50 pesados de más de dos ejes, cada nube con su recta de mínimos cuadrados y un rombo sobre cada recta a la velocidad de referencia, 80 km/h para los turismos y 70 km/h para las dos categorías de pesados; la leyenda da los niveles sonoros de vehículo de 78,9, 81,0 y 83,7 dB y el título el índice estadístico de paso de 80,2 dB"
  width="92%"
  loading="eager"
/>
...
El índice se usa casi siempre como diferencia con un **pavimento de referencia**
(9.5, apartado 10), y el apartado 10 ofrece cuatro: un hormigón asfáltico denso
real de árido máximo de 11 mm a 16 mm, con al menos un año de tráfico y la
textura y la absorción acotadas por la ISO 10844 y la ISO 13473-1 (10.1, el
caso general); un **caso de referencia normalizado** (10.2), un pavimento
...
pavimentos suecos para el rango de velocidad media, e imprime su media.
...
pasa a `statistical_pass_by` como índice o como sus tres niveles:
...
# 78.92 1.23   dB: este pavimento está 1,2 dB por encima de la referencia del anexo D
```
...
(`corrected_index_db`), que es lo que el apartado 13 pide informar junto al no
corregido. Donde la temperatura del aire se tomó en cada paso, 9.4 prefiere
...
83,8 dB) 80,1 dB, y una diferencia de 2,8 dB con un índice de referencia de
77,3 dB. La batería de conformidad lo reproduce todo, pero qué 79,9 sale
depende de un redondeo que el apartado deja abierto.

El apartado 9.2 termina: "All levels shall be calculated to two decimal places
and rounded to one decimal place." Las rectas impresas dan los niveles sonoros
de vehículo como 78,546, 81,114 y 83,838 dB. Llevados al índice sin redondear,
como hace esta biblioteca, dan 79,985 dB, que se informa como **80,0**; el
índice de los tres niveles tal como se imprimen, a un decimal, es 79,946 dB,
que es el **79,9** que imprime el anexo E. Así que el ejemplo alimenta el
índice con los niveles a un decimal de su propio informe, y redondea el índice
una sola vez: 79,946 dB llevado primero a dos decimales, 79,95, y luego a uno
también daría 80,0.
...
# 79.946   la cadena del ejemplo: el anexo E imprime 79,9
...
# 79.985   los niveles sin redondear: 80,0 una vez informado
```

La biblioteca lleva cada nivel a precisión completa y solo redondea lo que se
informa (las propiedades `reported_`, a un decimal, con el medio hacia arriba),
porque un nivel redondeado antes de usarlo es un nivel calculado a un decimal,
no a dos. Las dos cadenas son usos legítimos del método, y 9.5 describe la
segunda él mismo: los niveles informados son lo que permite a cualquiera
recalcular el índice. La diferencia es de 0,04 dB, por debajo de cualquier
precisión que el método reclame, pero basta para mover la última cifra
impresa; la fila de conformidad que fija los 79,9 dB ejecuta la cadena del
propio ejemplo.
...
y cuatro de hormigón asfáltico, que es exactamente la composición del anexo D;
pero los niveles del anexo D dan un índice de 78,92 dB con los mismos pesos, no
77,3. La norma no dice que sean la misma base de datos, así que la biblioteca
trata los 77,3 dB como el dato de entrada que se imprime. Y sus dispersiones de
...
(las tres categorías, los mínimos de 7.3, el índice de 9.5, los niveles
