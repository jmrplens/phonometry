...
exactamente con eso en mente: nunca remuestrea, nunca mezcla canales,
...
Los otros archivos que la biblioteca lee y escribe son catálogos de
materiales. La biblioteca publica las tablas de sus libros y normas y
ninguna de un fabricante, así que una hoja de características, una
declaración de prestaciones o un informe de ensayo propios van en un archivo
tuyo, un documento JSON o una hoja de cálculo guardada como CSV con una
cabecera JSON al lado, con la procedencia del documento: su tipo, su versión, el día en
que lo consultaste, el laboratorio y el informe. `read_catalogue` lee ese
archivo en filas de las mismas clases que entregan todos los catálogos
publicados, de modo que el núcleo poroso de tu hoja de características entra
en `medium()` igual que uno de un libro, y una cifra que la hoja de
características imprime en kPa·s/m² o en centímetros se convierte a partir
de sus dígitos y queda anotada tal como la imprimió. Los problemas de un
archivo se informan juntos, cada uno en su lugar del archivo, y nada de lo
que el archivo nombra se importa nunca.
`write_catalogue` escribe un catálogo de vuelta, y escribe una tabla
publicada con la misma forma, como plantilla de partida. `catalogue_schema`
escribe el esquema JSON de un archivo así, para que un editor lo complete
mientras lo escribes y marque lo que el lector rechazaría por su forma; el
sitio publica el esquema de todas las clases de fila que publica la
biblioteca.
...
- [Tus propios catálogos](/phonometry/es/io/material-catalogues/): una
  hoja de características, una declaración de prestaciones, un informe de
  ensayo o una medida tuya como archivo de catálogo: una tabla publicada
  como plantilla, el documento y su procedencia, una hoja de cálculo
  guardada como CSV, los rechazos que señalan una celda, las filas en los modelos y en todas las búsquedas por nombre,
  qué clase de fila guarda cada magnitud y el esquema JSON con el que un
  editor completa un archivo. Qué es cada número de una hoja
  de características, antes de teclearlo, está en
  [Cómo leer la hoja de características de un material](/phonometry/es/materials/reading-a-datasheet/),
  entre las guías de materiales.
...
sin decirlo sería un cambio de nivel silencioso. Es exactamente la clase de
valor por defecto que este módulo existe para rechazar.
</ScopeClaim>

<ScopeClaim status="not-covered">
**La biblioteca no trae datos de fabricantes.** Una hoja de características
se revisa sin cambiar de título, sus condiciones rara vez permiten copiarla, y
el valor que necesita un proyecto es el de la hoja de características con la
que se especificó, así que tus catálogos se quedan en tus archivos y en los
objetos que devuelve `read_catalogue`: la biblioteca no guarda ninguna copia ni abre ningún
enlace que nombre un archivo. Por lo mismo, una tabla de un fabricante pegada
en una issue o en una pull request nunca se incorpora.
</ScopeClaim>
</Scope>
...
primer archivo por la cadena. Después: todo lo demás. El sentido de la capa
de archivos es que los [niveles](/phonometry/es/signals/levels/levels/), el
