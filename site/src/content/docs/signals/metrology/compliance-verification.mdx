...
Five more functions carry the same `verify_` prefix and answer a different
...
its own standard writes its tolerances in. All five return a result object
...
- **What a measurement verdict is.** Narrower still. The five verifiers of the
...
the five verifiers that grade a measured instrument
rather than a design, and where each one's page is; how to read and cite the
...
that carries `.plot()` and an accredited `.report()` fiche. Five more verifiers grade a measurement rather
than a design and return a plain pass or fail:
`verify_aircraft_noise_system` against IEC 61265:1995, and `verify_weighting`,
`verify_phase_response`, `verify_signal_burst_response` and
`verify_running_rms_decay` against the tolerance tables of ISO 8041-1:2017.
