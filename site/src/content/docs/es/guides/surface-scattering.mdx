...
direccional $d(f)$ forma un `DiffusionSpectrum`, cuyo `report(path)` genera un
informe de ensayo de difusión de una página (ISO 17497-2, Cláusula 8.5): la tabla
por banda de $d$ junto a la curva $d(f)$ con eje de bandas, con el valor de
incidencia aleatoria $d$ (Cláusula 8.4) enmarcado. `verbose=True` añade la columna
del coeficiente normalizado $d_n$.

```python
import numpy as np
from phonometry import materials

# Coeficientes direccionales por banda d(f) reunidos en un espectro (aquí a partir
# de un ejemplo en forma cerrada; en la práctica cada d procede de
# directional_diffusion_coefficient sobre la respuesta polar de esa banda).
freqs = np.array([250, 500, 1000, 2000, 4000], float)
d = np.array([0.30, 0.45, 0.60, 0.75, 0.88])
spectrum = materials.directional_diffusion_spectrum(
    freqs, d, random_incidence=float(np.mean(d)))
...
<ReportPreview
	...
	description="Ficha de difusión direccional de una página: una cabecera de metadatos, la tabla por tercio de octava del coeficiente de difusión junto a la curva d(f) con eje de bandas y su acompañante normalizado d_n, y el coeficiente de difusión de incidencia aleatoria enmarcado sobre el rango de frecuencias ensayado."
	caption="Ficha de difusión direccional (DiffusionSpectrum.report), el espectro d(f) con el normalizado d_n."
/>
