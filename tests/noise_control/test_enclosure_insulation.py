...
def test_table_c1_reproduces_its_printed_pairs() -> None:
    assert TEST_ENVIRONMENT_REQUIREMENTS == oracle.ISO11546_2_TABLE_C1_DB
    assert "ISO 10204" not in TEST_ENVIRONMENT_REQUIREMENTS


def test_table_c2_reproduces_its_seven_rows() -> None:
    assert ROOM_ABSORPTION_ESTIMATES == oracle.ISO11546_2_TABLE_C2


@pytest.mark.parametrize("alpha", sorted(oracle.ISO11546_2_TABLE_C2))
def test_every_room_description_is_the_printed_one(alpha: float) -> None:
    assert ROOM_ABSORPTION_ESTIMATES[alpha] == oracle.ISO11546_2_TABLE_C2[alpha]
...
#
# ISO 11546-2 prints no worked example, so the numbers below come from
# documents that measured or worked the same quantities and owe nothing to
# this library: NPL CIRA(EXT) 009 (1996), five real rooms; Heisterkamp (2024),
# one BAuA workroom; Barron (2003) Example 7-8; Peters, Smith and Hollins
# (2011) Example 1.13; Schirmer (2006) 10.8.1; and IFA-LSA 01-243 (2014). The
# numbers themselves, with the edition, the PDF page and the folio of each,
# are in ``tests/reference_data/enclosure_cabin_insulation.py``, which the
# conformance rows read too.
# ---------------------------------------------------------------------------

BARRON_OCTAVES_HZ = np.array(oracle.BARRON_TABLE_7_5_OCTAVES_HZ)
BARRON_WITHOUT_DB = np.array(oracle.BARRON_TABLE_7_5_LP_WITHOUT_DB)
BARRON_WITH_DB = np.array(oracle.BARRON_TABLE_7_5_LP_WITH_DB)
BARRON_TOTALS_DBA = (
    oracle.BARRON_EXAMPLE_7_8_LPA_WITHOUT_DBA,
    oracle.BARRON_EXAMPLE_7_8_LPA_WITH_DBA,
)
SMITH_OCTAVES_HZ = np.array(oracle.SMITH_EXAMPLE_1_13_OCTAVES_HZ)
SMITH_ATTENUATION_DB = np.array(oracle.SMITH_EXAMPLE_1_13_ATTENUATION_DB)
...
def test_the_npl_absolute_factors_are_the_measured_power_less_the_source() -> None:
    ...
    for room, printed in oracle.NPL_MEASURED_K2A.items():
        for surface, want in printed.items():
            level = oracle.NPL_SOUND_POWER_DB[room][surface]
            assert round(level - oracle.NPL_REFERENCE_SOURCE_DB, 1) == want
...
def test_the_npl_room_e_area_conflict_decides_two_of_its_three_surfaces() -> None:
    ...
    agreeing = {}
    for room_surface_m2 in (
        oracle.NPL_ROOMS["E"][0],
        oracle.NPL_TABLE_6_ROOM_E_SURFACE_M2,
    ):
        agreeing[room_surface_m2] = [
            ...
            is (measured <= 7.0)
            for surface, measured in oracle.NPL_MEASURED_K2A["E"].items()
        ]
...
def test_the_npl_estimated_absorption_column_drops_the_factor_four() -> None:
    ...
    for room, printed in oracle.NPL_ESTIMATED_K2A.items():
        ...
        for surface, bounds in printed.items():
            surface_m2 = oracle.NPL_SURFACE_M2[surface]
...
def test_the_baua_engineers_reach_the_printed_environmental_corrections() -> None:
    for name, (alpha, room_surface_m2, _, *printed) in oracle.BAUA_ENGINEERS.items():
        for surface_m2, want in zip(oracle.BAUA_SURFACES_M2, printed, strict=True):
            got = emission.environmental_correction(
                surface_m2,
                mean_absorption_coefficient=alpha,
                room_surface=room_surface_m2,
...
def test_the_baua_absorption_areas_are_alpha_times_the_boundary_surface() -> None:
    ...
    for name, (alpha, room_surface_m2, want, *_) in oracle.BAUA_ENGINEERS.items():
...
def test_the_baua_direct_method_absorption_areas_give_the_printed_factors() -> None:
    ...
    for name, (absorption_m2, *printed) in oracle.BAUA_DIRECT.items():
...
def test_the_baua_reverberation_room_level_does_not_fit_its_absorption_area() -> None:
    ...
    def absorption_area(level_db: float) -> float:
        surface_m2 = oracle.BAUA_DIRECT_SURFACE_M2
        ratio = surface_m2 * 10.0 ** (0.1 * (level_db - oracle.BAUA_SOURCE_POWER_DB))
        return 4.0 * surface_m2 / (ratio - 1.0)

    assert absorption_area(oracle.BAUA_LEVEL_IN_SITU_DB["workroom"]) == pytest.approx(
        oracle.BAUA_DIRECT["workroom"][0], abs=0.1
    )
    astray = absorption_area(oracle.BAUA_LEVEL_IN_SITU_DB["former reverberation room"])
    assert astray == pytest.approx(57.1, abs=0.1)
    assert astray != pytest.approx(
        oracle.BAUA_DIRECT["former reverberation room"][0], abs=1.0
    )
...
def test_the_smith_example_gives_one_enclosure_two_a_weighted_values() -> None:
    ...
    for name, (_, want) in oracle.SMITH_EXAMPLE_1_13_MACHINES.items():
...
def test_definition_3_14_counts_the_opening_in_the_interior_surface() -> None:
    ...
    assert ratio == pytest.approx(oracle.SCHIRMER_LEAK_RATIO, abs=5e-5)
