...
so a laboratory within its maximum cannot pass an instrument outside its
tolerance. Both limits are inclusive: a deviation equal to its acceptance limit
...
each. IEC 60942 writes its limits on the absolute deviation, so its examples
take one number (`acceptance_limits=0.25` is ±0.25 dB); IEC 61672-1 writes most
of its own as an asymmetric pair. `verify_sound_calibrator` applies the rule to
