...
mid-frequency reverberation time is supplied through `requirement` (read as the
maximum acceptable $T_\text{mid}$, the usual form of a classroom or open-plan
limit). It uses the same `ReportMetadata` container and rendering engine as the
