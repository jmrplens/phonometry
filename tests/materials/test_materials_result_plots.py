...
def test_transfer_matrix_plot_tl_and_absorption() -> None:
    ...
    ax = tm.plot(frequency=f, characteristic_impedance=rho_c)
...
def test_transfer_matrix_plot_forwards_kwargs_and_composes() -> None:
    tm, f, rho_c = _transfer_matrix()
    ax = tm.plot(frequency=f, characteristic_impedance=rho_c, linewidth=2, color="red")
    line = ax.lines[0]
    ...
    out = tm.plot(frequency=f, characteristic_impedance=rho_c, ax=external)
...
def test_transfer_matrix_plot_spanish_and_bad_language() -> None:
    tm, f, rho_c = _transfer_matrix()
    ax = tm.plot(frequency=f, characteristic_impedance=rho_c, language="es")
    ...
    with pytest.raises(ValueError, match=r"Unknown language"):
        tm.plot(frequency=f, characteristic_impedance=rho_c, language="fr")
...
def test_transfer_matrix_plot_rejects_mismatched_frequency() -> None:
    ...
    with pytest.raises(ValueError, match=r"TransferMatrix\.plot: 'frequency'"):
        tm.plot(frequency=f[:-3], characteristic_impedance=rho_c)
    plt.close("all")
...
def test_transfer_matrix_plot_rejects_nan_impedance() -> None:
    ...
    with pytest.raises(ValueError, match="'characteristic_impedance' must be positive"):
        tm.plot(frequency=f, characteristic_impedance=float("nan"))
    plt.close("all")
