...
la secuencia de cinco pozos seis veces (`repetitions=6`, un panel de 2,1 m):
...
panel = materials.metadiffuser_reflection(f, pozos, depth=0.02, period_m=0.07)
...
#     pozos, depth=0.02, period_m=0.07).
```
...
meta = materials.metadiffuser_polar_response(2000.0, pozos, depth=0.02, period_m=0.07,
                                             repetitions=6)
...
qrd = materials.predict_diffuser_polar_response(
    0.07, 2000.0, depths=profundidades, repetitions=6, include_obliquity=False,
)
...
<ThemeImage src="https://raw.githubusercontent.com/jmrplens/phonometry/main/.github/images/diagram_metadiffuser_chain.svg" alt="Tres bandas, de arriba abajo. Primero, un periodo del difusor de residuo cuadrático objetivo, diseñado para 500 hercios, dibujado en sección con pozos de 6,9, 27,4, 27,4 y 6,9 centímetros de profundidad y un pozo plano, junto a una tabla con la secuencia 1, 4, 4, 1, 0, esas profundidades y la fase que cada pozo tiene que devolver a 2 kilohercios: más 72, menos 72, menos 72, más 72 y 0 grados; a la derecha, a la misma escala, el metadifusor de 35 por 2 centímetros con sus cinco ranuras. Segundo, la ranura 1 ampliada, de 14,7 milímetros de alto y 20 milímetros de profundidad, con dos resonadores de Helmholtz separados 10 milímetros y alojados en el tabique, y a su lado la cadena de matrices de transferencia dispuesta a lo largo de la profundidad: la corrección de radiación en la boca, medio paso de red de ranura a cada lado de cada resonador, los resonadores en derivación y el respaldo rígido, que un recuadro convierte en el coeficiente de reflexión de la ranura; una flecha vuelve a la tabla, ajustando la geometría ranura a ranura hasta que la fase de cada reflexión es su objetivo. Tercero, el periodo repetido seis veces en un panel de 2,1 metros bajo un semicírculo de receptores cada 5 grados, con cinco lóbulos de red cerca de 0, más y menos 29 y más y menos 78,5 grados, y tres recuadros con el campo lejano de Fraunhofer, las direcciones de los lóbulos y el coeficiente de difusión normalizado frente a un panel plano del mismo ancho" width="100%" />

*Toda la predicción en una figura, para el panel publicado. Los residuos
cuadráticos fijan una fase por pozo; después cada ranura se ajusta, a través de
la cadena de matrices de transferencia, hasta que devuelve esa fase desde 20 mm
de profundidad; y solo entonces pasan seis periodos por el campo lejano hasta
un coeficiente de difusión. Los dos paneles de arriba comparten escala: 27,4 cm
de pozos frente a 2 cm de ranuras.*
...
barrida = materials.metadiffuser_reflection(barrido, pozos, depth=0.02, period_m=0.07)
...
qrd = materials.predicted_diffusion_spectrum(
    0.07, bandas, depths=profundidades, repetitions=6, include_obliquity=False)
