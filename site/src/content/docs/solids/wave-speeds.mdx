...
solve. `PUBLISHED_SOLIDS` holds two hundred and nineteen rows read from six published tables, keyed
...
`board.derived["youngs_modulus_pa"]` names the three cells it came from. A
number this library computed is never stored as though a page had printed it.

### The key names the book on purpose

Four of the six tables print a row named Steel, and they are not the same
...
one per cent carbon, and Long a speed and a density and no modulus at all.
...
Long prints no modulus at all, which is why the loop asks whether there is one
before dividing. A book that gives fewer columns is the normal case here, not
the exception.
...
further than Hopkins trusts it.
...
different material. Every density the gate does not already accept agrees
across these six tables to within 2,6 per cent, while the moduli disagree by
nineteen per cent on aluminium and thirty on lead without anybody being wrong.
...
The first full run found tin, where three books do not agree about a factor of
twelve.
...
reproduces his printed 2 720 m/s exactly; and Mechel's `h f_c` of 81 m Hz
follows from 4,4 and not from 54. No page contradicts itself, so this is not an
errata to register and the catalogue keeps all three readings.
...
other two hundred rows argue against, which is the whole reason nothing in this
