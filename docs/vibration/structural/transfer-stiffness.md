...
through the effective mass of section 5; the clause 7.6 linearity criterion is
described here and left to the operator.
...
# Indirect method on an 8 kg blocking mass: below 189 Hz |T| > 0.1, and those
# lines never reach the average.
...
0,289, 1,155 and 0,433 dB, not the 0,3, 1,2 and 0,5 dB of Table B.1: 0,5 dB
is not the nearest tenth of 0,433 dB (see the [errata](../../ERRATA.md)), and
every term can be passed in to reproduce the table.
...
the 7.6 linearity criterion; Parts 4 and 5 print no worked example, and their
rows are closed forms too (a band of identical lines averages to itself, a
massless spring gives a flat stiffness, the output mass on its bound biases
the force by 0,5 dB).
