...
Before any of them, [Start](start/index.md) is the six pages meant to be read
...
[Upgrading from 3.3 to 4.0](start/upgrading.md) is the map for code written
...
- [Your own catalogues](io/material-catalogues.md): a data sheet, a test report or a measurement of yours as a catalogue file, read into the rows every published catalogue hands out, searched with the same lookups, handed to the same models and completed in your editor with its JSON Schema
...
- [Spatial sound decay in workrooms (ISO 14257, ISO 11690-3)](buildings/rooms/workroom-sound-decay.md): the spatial sound distribution curve of a workroom, the rate of spatial decay $\mathrm{DL}_2$ and the excess $\mathrm{DL}_\mathrm{f}$ over a free field, the Annex B correction for the source's own curve, and the prediction categories and detail levels that match them at design time
...
- [Suspended ceilings: the EN 16487 test code](materials/absorbers/suspended-ceilings.md): the specimen size, the type E mounting depth and the air-absorption cap that make one ceiling measurement comparable with another, and Table 1, the reproducibility between European laboratories
...
- [Dynamic transfer stiffness of resilient elements](vibration/structural/transfer-stiffness.md): the ISO 10846 dynamic transfer stiffness $k_{21}$ of vibration isolators: the level $L_k$ re 1 N/m and loss factor, the direct and indirect (transmissibility) determination methods, the one-third-octave band average every part asks for, the adequacy checks of the rig and the effective blocking mass (ISO 10846-4), the driving-point method with its upper limiting frequency and Annex B budget (ISO 10846-5), and the Annex-A relation to mechanical impedance and effective mass
...
- [Verifying a human-vibration meter](vibration/human/meter-verification.md): the ISO 8041-1:2017 tolerances on the frequency weightings, with the four transition frequencies of Table 4, the three distinct tolerance pairs Table 5 prints across them, the reference conditions of Table 1 and the indication tolerance of Table 2, giving a vibration meter the kind of verdict IEC 61672-1 gives a sound level meter, and the personal vibration exposure meter of ISO 8041-2:2021, graded with the same burst tables, its own two-row Table 2 and its own uncertainty limits
...
- [Vibration and people in buildings (DIN 4150-2)](vibration/immission/people-in-buildings.md): the guide values of Table 1 by area, time of day and source, the procedure of 6.2 as a verdict with the criterion that decided it, the assessment vibration severity of Formulae (4) and (5) with the rest-time weight and the exposure A_r allows, the rules of 6.5 for rare events, roads, railways and construction sites with Table 2 and the interpolation of Figure 3, the railway classes of Annex A with the interval their spread puts on the result, the trains an hour of Figure D.1, and the estimate of Clause 7 from a peak velocity, with Formula (A.1b) in the corrected form registered in the errata
- [Railway vibration by category of train (E DIN 4150-2)](vibration/immission/railway-categories.md): the draft of August 2023 that is to replace DIN 4150-2, with one clock interval per passage, a KB_Fmax that is 1,5 times the r.m.s. of a category, the weighting factors of Table 2 by kind of train and alignment, the night-time upper value of a new line, the 25 % rule of an altered one, and the rest of the draft's changes to Table 1, the procedure and the sources behind an edition switch
- [Predicting railway vibration (E DIN 45672-3)](vibration/immission/railway-prediction.md): the additive chain of Formula (1) from an emission spectrum through the ground, the foundation, the floor and a mitigation to a third-octave spectrum on a floor, the speed rescaling and the ground transmission of Clauses 5.2 and 5.3, the six transfer tables of Annex A, the KB values of Clause 7 formed from the spectrum, and Annex B on a train as a line of point sources, with what the Annex C print gets wrong
- [Predicting vibration before measuring (DIN 4150-1)](vibration/immission/vibration-prediction.md): the far field of Formula (1), the decay of Formula (2) with the exponents of Figure 1 and the damping of the ground, the building on its ground and its floors of Clause 4.3, the blast, the falling mass and the machine hall of Formulae (5) to (7) with the nomogram of Figure 3 read off the page, and what a track excites at
...
- [Sound power in the 16 kHz octave](devices/emission/sound-power-high-frequency.md): the four ISO 9295 methods for the octave band above ISO 3741 and ISO 3744, with the room constant from the measured reverberation time or from the air absorption of Annex A, the comparison for broadband noise and for tones, and the free-field correction beyond 2 m
...
- [Measuring a silencer](devices/noise-control/silencer-measurement.md): the substitution measurement a catalogue insertion loss comes from, its reverberation correction, the octave fold done on the transmitted energy, the three-or-five microphone rule, and the reproducibility both standards publish (ISO 7235, ISO 11691)
- [Enclosures and cabins measured](devices/noise-control/enclosure-cabin-insulation.md): what a built enclosure is actually worth, as the difference of two sound power determinations, with Table 1 deciding what may be declared from which method and Annex C asking whether the room is good enough at all; and the cabin, the inverse problem, whose in-situ source-position count is read off the spread of its own answer (ISO 11546-1, ISO 11546-2, ISO 11957)
- [Silencers, screens and barriers in situ](devices/noise-control/in-situ-noise-control.md): the silencer measured in the duct it was fitted to, the screen wheeled away for one of the two runs, and the outdoor barrier that was built before anyone thought of measuring it, with the twenty installations of Figure 1, the reference microphone that normalises the source, and the three different corrections for background noise that must never be merged into one (ISO 11820, ISO 11821, ISO 10847)
...
- [Published catalogues](https://jmrplens.github.io/phonometry/reference/catalogues/): the material and fluid values this library reads rather than computes, in one table per catalogue, each row naming the book, the table, the PDF page and the printed folio it was read on. Generated from the packaged data files (`make catalogue-data`), so it has no hand-written mirror here, the same as the generated API reference
- [Report fiches](https://jmrplens.github.io/phonometry/reference/reports/): every PDF fiche `.report()` renders, grouped by area, each linking to the fiche and to the guide that explains the measurement behind it. Built from the `<ReportPreview />` each guide already carries beside the fiche it shows, so it has no hand-written mirror here either: a second list would be a second thing to keep in step with the guides, which is the one failure this page is built to prevent
