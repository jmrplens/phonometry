...
fs = 8192.0
period_s = 1.0 / 32.0        # una revolución: 256 muestras a esta frecuencia
m, n_avg = 256, 40
...
period_s = 1.0 / 32.0
signals.comb_filter_response(np.array([16.0 / period_s]), period_s, 8)   # 1.0 en un diente
signals.comb_filter_response(np.array([0.25 / period_s]), period_s, 2)   # 1/sqrt(2)
signals.comb_filter_response(np.array([0.5 / period_s]), period_s, 2)    # 0.0 en un nodo
```
...
la suposición escondida detrás de un `period_s` escalar es que el eje mantiene ese
...
```python
fs = 8192.0
period_s = 1.0 / 31.7                       # fs * period_s no es entero
t = np.arange(int(40 * period_s * fs)) / fs
recording = np.cos(2.0 * np.pi * t / period_s)  # un ciclo por revolución
