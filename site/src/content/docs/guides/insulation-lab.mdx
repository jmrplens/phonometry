...
<ReportPreview
	...
	description="One-page floor-covering impact-improvement test report for a soft carpet measured on a small mock-up: the metadata header (client, floor-covering description, mounting, mass per area, climate), the one-third-octave delta-L table beside the delta-L(f) improvement curve rising with frequency, the boxed delta-Lw (CI,delta) = 19 (-11) dB weighted improvement (ISO 717-2), and a PASS verdict against the 17 dB requirement (a higher weighted improvement is better)."
