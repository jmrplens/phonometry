...
path), so the numbers are verifiable rather than tuned. A closing section
...
The same panels are available interactively through `.plot()`, which draws the
IEC 60268-5 data sheet (the wide on-axis response above the impedance, THD and
polar panels) sharing its drawing code with the report, so the two never
diverge. Passing an axes draws only the on-axis response on it.

```python
result.plot()   # multi-panel IEC 60268-5 data sheet (needs matplotlib)
```

<ThemeImage src="https://raw.githubusercontent.com/jmrplens/phonometry/main/.github/images/loudspeaker_characteristics.svg" alt="IEC 60268-5 loudspeaker data sheet: a wide on-axis sound-pressure-level response with its shaded tolerance band and effective-range markers on a nominal-frequency axis, above the impedance modulus with its rated and 80 %-of-rated lines, the total harmonic distortion against frequency, and the polar directivity on the 25 dB reference circle" width="92%" />
...
The same panels are available interactively through `.plot()`, which draws the
IEC 60268-4 data sheet (the wide free-field response above the polar, noise and
distortion panels) sharing its drawing code with the report. Passing an axes
draws only the free-field response on it.

```python
result.plot()   # multi-panel IEC 60268-4 data sheet (needs matplotlib)
```

<ThemeImage src="https://raw.githubusercontent.com/jmrplens/phonometry/main/.github/images/microphone_characteristics.svg" alt="IEC 60268-4 microphone data sheet: a wide free-field relative-response with its shaded tolerance band, reference-frequency marker and effective-range markers on a nominal-frequency axis, above the cardioid polar directional pattern on the 25 dB reference circle, the inherent-noise band-level spectrum, and the total harmonic distortion against sound pressure level with its limit and overload lines" width="92%" />
