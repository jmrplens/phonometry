...
from phonometry.compliance import verify_filter_class  # noqa: E402
...
class WeightingDeviation:
    """Worst-case weighting deviation and remaining class-1 headroom."""
...
def _weighting_deviation(curve: str, fs: int) -> WeightingDeviation:
    """Worst-case digital-vs-normative deviation and class-1 headroom.
    ...
    normative headroom is the IEC 61672-1 Table 3 class-1 acceptance band;
...
def _weighting_check(curve: str, fs: int) -> Outcome:
    ...
    return Outcome(
        expected="within class-1 limits",
        computed=f"worst {res.worst_dev:+.3f} dB @ {res.worst_freq:.0f} Hz",
...
@register(
    ...
    "Equal-loudness contour, 60 phon @ 2 kHz",
)
def _chk_iso226_contour() -> Outcome:
    freqs, spl = ph.equal_loudness_contour(60.0)
    computed = float(spl[np.asarray(freqs) == 2000.0][0])
    return numeric(60.0, computed, 0.05, unit="dB SPL", places=3)
...
def _numerical_validation_section() -> str:
    ...
    lines.append("")
    lines.append(
        "IEC 61260-1:2014 class verification per filter architecture "
        "(order 6, one-third-octave, 100 Hz-10 kHz, fs = 48 kHz). A positive "
        "margin means the class-1 acceptance limits are met with that much room."
    )
    lines.append("")
    lines.append("| Architecture | Overall class | Min class-1 margin | Min class-2 margin |")
    lines.append("|:---|:---:|:---:|:---:|")
    for arch in _FILTER_ARCHS:
        fc = _filter_class(arch, 3)
        cls_txt = f"class {fc.overall_class}" if fc.overall_class else "none"
        lines.append(
            f"| {arch} | {cls_txt} | {fc.min_margin1:+.3f} dB "
            f"| {fc.min_margin2:+.3f} dB |"
        )
    lines.append("")
    lines.append(
        "Frequency-weighting worst-case deviation of the designed digital "
        "filter from the normative curve, with the remaining class-1 / "
        "tolerance headroom (A/C: IEC 61672-1 Table 3; G: ISO 7196 A.3)."
    )
    lines.append("")
    lines.append("| Curve | fs | Worst deviation | Min headroom |")
    lines.append("|:---|:---:|:---:|:---:|")
