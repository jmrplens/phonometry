...
El material no tiene por qué ser un nombre. `material=` también admite una
fila `PlateauMaterial`: una de `solids.PUBLISHED_PLATEAU_DATA`, que dibuja la
misma curva que su nombre, o una fila tuya que `io.read_catalogue` haya leído
de una hoja de características
([Tus propios catálogos](/phonometry/es/io/material-catalogues/)). Una fila
entrega sus tres números solo a través de `printed()`, así que una hoja que declara la altura de la meseta
como una cota («no más de 30 dB») se rechaza con lo que imprime en lugar de
dibujarse a 30 dB, y un `plateau_height` que pases sustituye al de la fila
sin llegar a leerlo.
...
- `method="integral"` evalúa numéricamente esa integral doble. Es la única vía
  ...
  `limiting_angle_deg` fijo (78 grados por defecto, el valor de Sharp, que es el
...
borde empotrado radia más que uno simplemente apoyado, y una placa con pantallas
perpendiculares a sus bordes más que una enrasada en una pantalla infinita: los
...
1,74. La versión española vigente de la ISO 12354-1 llama «bafle infinito» a esa
pantalla infinita; aquí se dice «pantalla», que es el término del vocabulario
UNE 21302-801:2001 en su entrada 801-27-16 y el que usaba la adopción anterior
de la misma norma europea.
...
# Una línea — la placa en pantalla que supone el modelo:
...
| `boundary` / `baffle` | str | — | `'simply_supported'`/`'clamped'`, `'infinite'`/`'perpendicular'` | Constantes de borde y pantalla |
