...
damping_treatments_named(
    name: str,
    *,
    catalogue: Mapping[str, DampingTreatment] | None = None,
...
| Exception | When |
| :--- | :--- |
| TypeError | for a *catalogue* that is not a mapping, or that holds a row that is not a [`DampingTreatment`](/phonometry/reference/api/solids/damping-treatments/#dampingtreatment), naming its key. |
