...
(PVEM): the one a worker wears through the shift, which has to log the
exposure for at least 12 h without anyone touching it (5.1 and 5.12). It
measures the same quantities through the same weightings, and Part 2 does not
write a second set of tables for it. It prints the Part 1 tables again and
changes only what a meter worn on the body needs changed, so the question for
this section is which of the numbers above a PVEM is held to, and which are
its own.
