...
the molar mass and the ratio of specific heats, which between them fix every
state it can be in. `fluids.PUBLISHED_GASES` holds those two columns, and
`Gas.ideal_state` walks from them to whichever state is asked for, carrying the
page along with the number. Air is in both tables below, because both books
