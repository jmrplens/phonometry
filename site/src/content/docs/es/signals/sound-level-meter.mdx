...
envelope = filters.time_weighting(weighted, fs, mode="fast")  # valor cuadrático medio en Pa^2
...
```python
spl_a = filters.octave_filter(weighted, fs, fraction=3, nominal=True).levels
...
```python
centres = filters.octave_filter(recording, fs, fraction=3).frequencies  # centros exactos
...
propia cobertura en detalle. El calibrador del que parte la cadena tiene su
propio veredicto: `metrology.verify_sound_calibrator` juzga lo que un
laboratorio midió en él frente a cada requisito de IEC 60942:2017 que tiene
límite de aceptación, con la regla de conformidad del IEC TC 29.
</ScopeClaim>
...
paso 7 criban la *grabación*, no el instrumento. Lo mismo vale para el
calibrador: aquí no se realiza sobre él ningún ensayo de IEC 60942:2017.
`verify_sound_calibrator` juzga los números que midió un laboratorio, y
`sensitivity()` criba la estabilidad a corto plazo de la toma frente a la
Tabla 2; la [guía de calibración](/phonometry/es/signals/metrology/calibration/#verificar-el-propio-calibrador-iec-609422017)
dice exactamente qué comprueba cada una y qué no.
</ScopeClaim>
</Scope>
