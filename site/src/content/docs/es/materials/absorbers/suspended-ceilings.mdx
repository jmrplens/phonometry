...
separados 1,2 m por lo menos; un marco de montaje de material macizo de 20 kg/m² o
más; y una muestra que no flecte más de 5 mm en ningún punto. Los tres que recibe
la comprobación, la sección de la subestructura, la densidad del marco y la
flecha, sí se juzgan, y salirse de uno lanza un `SuspendedCeilingWarning` que dice
cuál. El paso de la subestructura y la sección y separación de los soportes se
publican como `SUBSTRUCTURE_SPACING_M`, `SUPPORT_SECTION_MM` y
`MIN_SUPPORT_SPACING_M` para construir con ellos: describen cómo se colgó la
muestra y no una medida hecha sobre ella, así que nada en la llamada puede
comprobarlos.
