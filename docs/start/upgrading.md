...
| an empty `name` or `source` | `CatalogueError` |
...
| `bounded_above` or `bounded_below` on a field with no range; a range with an end that is not finite, or with its low end above its high one; an `uncertainty` below zero | `CatalogueError` |
...
| a density, mass, speed, pressure or modulus, flow resistivity, stiffness per area, length or area below zero; a porosity outside 0 to 1; a share of a whole outside 0 to 100 per cent | `CatalogueError`; a Celsius temperature, a decay rate per metre and a level in decibels stay signed |
...
| a subclass field annotated with a type the contract has no check for | `TypeError` the first time the class is built; a field is a number or a whole number that may be `None`, a `bool`, a `str`, a `frozenset[str]` or a `Mapping[str, ...]` of those |
...
| `GroundSurface.porosity` holding 36.5 for six rows of Cox & D'Antonio Table 6.7 | `porosity_percent`, with the same digits; `porosity` is left empty on those six |

No published row changes value: every packaged row already met the contract
except those six porosities, which Cox prints in per cent in a column of
fractions, and every published porous row with both moduli prints its loss
factor.
