...
print(result.range_limited)          # True: its top bands reach Nyquist
...
to $\Omega \approx 18$ or more, past the end of the Table 1 mask (at least
70 dB for class 1 from $\Omega \approx 5.39$ up, the octave $G^4$ row carried
...
lies below $5.39\,f_\mathrm{m}$, and the 20 kHz band is evaluated only to
