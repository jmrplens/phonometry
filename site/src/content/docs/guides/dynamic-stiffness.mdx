...
loaded thickness $d$, test facility, climate), a compact metrics table (the
...
`language="es"` renders the Spanish fiche. It needs the report extra
...
)   # one-page fiche (needs phonometry[report])
```
