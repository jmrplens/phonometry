...
$r_P$) junto a la propia gráfica de decaimiento espacial del resultado
(`.plot()`, la regresión del apartado 6.2 en el eje logarítmico de distancia
con la lectura a 4 m y los cruces $r_D$ / $r_P$ marcados, la curva al estilo de
la Figura 3), el $D_{2,S}$ enmarcado con las demás magnitudes al lado, y un pie
...
`metadata=None` se obtiene una ficha de caracterización simple. El renderizado
necesita reportlab (`pip install phonometry[report]`); solo se admite
