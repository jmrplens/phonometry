...
references:
  ...
  - type: standard
    ...
    note: "La rigidez en el punto de excitación k₁,₁ de la Fórmula (3), la frecuencia límite superior del 6.2, las medias en banda de las Fórmulas (6) y (7), las Desigualdades (1) y (2) y el presupuesto del Anexo B. Tampoco trae ejemplo resuelto; el término de linealidad de la Tabla B.1 es 0,5 dB donde su expresión da 0,433 dB (véanse las erratas)."
---
...
requisitos de equipo que deciden si hay resultado siquiera. Los dos que son
diferencias de nivel pueden juzgarse a partir de los espectros medidos
(sección 5); el resto se deja al operador.
...
* **Un ensayo previo de unidireccionalidad** (6.4, Desigualdad 3). La
  ...
  `check_unwanted_input` las juzga.
...
# Método indirecto con una masa de bloqueo de 8 kg: por debajo de 189 Hz
# |T| > 0,1, y esas líneas nunca llegan a la media.
...
# Una placa de fuerza de 0,4 kg bajo una fuerza de salida de 1 N (120 dB re 1 µN)
# y una aceleración de salida de 0,1 m/s² (100 dB re 1 µm/s²):
...
7.6.1. Su `.band_average()` promedia solo las líneas válidas, todas por debajo
de $f_\mathrm{UL}$; como cada una está dentro de 2 dB de $k_{2,1}$, también lo
está cada banda. Por debajo de 20 Hz el 7.5 pide un paso de 0,2 Hz entre
...
# una placa de fuerza de 2 kg: los transductores leen la inercia de la placa con
# el elemento.
...
res.plot(language="es")             # el nivel, el umbral de 2 dB y f_UL
...
*Dónde se detiene el método. La placa de fuerza es de 2 kg, así que la rigidez
medida es $k_{2,1} - \omega^2 m_0$ y se aparta de la rigidez de transferencia
según sube la frecuencia; a 52,2 Hz está 2 dB por debajo de su valor de 1 Hz a
...
expresiones que imprime el anexo, no los 0,3, 1,2 y 0,5 dB a los que los
redondea la Tabla B.1: 0,5 dB no es la décima más cercana a 0,433 dB, como
registran las [erratas](/phonometry/es/reference/errata/), y cualquier término
puede pasarse como argumento para reproducir la tabla o para declarar una
estimación razonada. Domina el término de discrepancia: es el precio de medir
...
medida. El criterio de linealidad del apartado 7.6 (dos espectros de entrada
separados 10 dB dentro de 1,5 dB) no se comprueba en el código, ni tampoco el
ensayo previo de fondo, la planitud de 0,5 dB de los transductores y el
emparejamiento entre canales, ni el paso de 0,2 Hz entre líneas que la
ISO 10846-5 pide por debajo de 20 Hz. Los soportes hidráulicos quedan del
