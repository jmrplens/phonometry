...
métricas solo con NumPy y SciPy. Esto incluye leer con `phonometry.io` todo
...
print(f"Bandas: {result.frequencies}")
...
# Aplicar el banco de filtros de 1/3 de octava
filtered = filters.octave_filter(y, fs=fs, fraction=3, limits=[12.0, 20000.0])
...
dos WAV: la toma del calibrador y la medición. `phonometry.io` lee los
...
multicanal EXTENSIBLE, RF64 nocturno. Avisa si el archivo es una copia
...
El análisis en octavas de arriba usa `phonometry.filters`, uno de los veintiún
