...
its own standard writes its tolerances in. Four of them return a result object
whose `passes` says so and whose other fields say where and by how much;
`verify_running_rms_decay` grades one printed row and returns the `bool`.
