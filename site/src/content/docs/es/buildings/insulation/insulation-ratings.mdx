...
<ThemeImage src="https://raw.githubusercontent.com/jmrplens/phonometry/main/.github/images/diagram_iso717_rating_chain.svg" alt="Cadena de cálculo de los índices globales de ISO 717. Arriba, una tira de bandas de tercio de octava de 50 Hz a 5000 Hz: las 16 bandas de 100 Hz a 3150 Hz en trazo continuo con la de 500 Hz destacada, las cinco bandas de octava de 125 Hz a 2000 Hz marcadas con corchetes debajo, y las bandas de 50, 63, 80, 4000 y 5000 Hz a trazos, como rango ampliado que solo leen los términos de adaptación, las de 4000 y 5000 Hz solo en ISO 717-1. Un recuadro enuncia la regla que comparten las dos partes: desplazar la curva de la Tabla 3 hacia la curva medida en pasos de 1 dB hasta que la suma de desviaciones desfavorables sea lo mayor posible pero no más de 32,0 dB en 16 bandas o 10,0 dB en 5, en pasos de 0,1 dB para expresar la incertidumbre, y leer la curva desplazada a 500 Hz. Debajo, dos gráficas pequeñas lado a lado. A la izquierda, ISO 717-1 para ruido aéreo: la curva de referencia bajada 22 dB sobre un índice de reducción acústica medido, con barras donde la medida queda por debajo que suman 31,8 dB, y de ahí Rw = 52 − 22 = 30 dB; C = 28 − 30 = −2 dB con el espectro n.º 1, ruido rosa, y Ctr = 27 − 30 = −3 dB con el espectro n.º 2, tráfico urbano, ambos sobre las 16 bandas, expresado como Rw(C;Ctr) = 30 (−2; −3) dB. A la derecha, ISO 717-2 para ruido de impactos: la curva de referencia subida 19 dB sobre un nivel de impactos normalizado medido, con barras donde la medida queda por encima que suman 28,0 dB, y de ahí Ln,w = 60 + 19 = 79 dB, con 5 dB menos cuando las bandas son de octava, y CI a partir de la suma energética sin ponderar de las 15 primeras bandas, sin la de 3150 Hz. Cada lado lista sus términos de rango ampliado, y un recuadro al pie lleva Cj = XAj − Xw y CI = Ln,sum − 15 − Ln,w" width="100%" />

*Una regla, dos motores. Las dos partes desplazan igual su propia curva de la
Tabla 3, y el desplazamiento solo cambia entre ellas en el lado de la curva en
que cuenta una desviación: por debajo en el aislamiento a ruido aéreo, donde la
curva se detiene 22 dB más abajo con 31,8 dB de desviaciones y lee 30 dB a
500 Hz, y por encima en el ruido de impactos, donde se detiene 19 dB más arriba
con 28,0 dB y lee 79 dB. Cuando las bandas son de octava, ISO 717-2 resta otros
5 dB a esa lectura y ISO 717-1 no. Los términos de adaptación vuelven después a
las bandas medidas: las 16 para $C$ y $C_\mathrm{tr}$, y solo las 15 primeras
para $C_\mathrm{I}$.*
...
Sube 3 dB por tercio de octava (9 dB por octava) de 100 Hz a 400 Hz, después
1 dB por tercio de octava hasta 1250 Hz, y es plana a partir de 1250 Hz. Un
motor de valoración que lleve esa forma es deliberadamente indulgente en baja
frecuencia e implacable por encima de 1 kHz, que es la razón por la que una
construcción con un valle en baja frecuencia puede valorarse bien, y la razón
por la que existe $C_\mathrm{tr}$: para devolverle la penalización. El objeto de
resultado entrega la curva en su posición desplazada, así que la curva sin
desplazar está a una resta de distancia:
...
sí solo $L_\mathrm{n,w} = 79$ dB. El corolario es una trampa de diseño: un
...
# Ejemplo resuelto del Anexo C de ISO 717-2: el forjado pesado desnudo de la
# Tabla C.1, Ln de laboratorio en tercios de octava. CI se lee sobre la suma
# de 100-2500 Hz que pide A.2.1, no sobre las dieciséis bandas que suma la
# edición de 2020 de esa celda.
...
# 16 niveles de impactos Ln en tercios de octava, dB (ejemplo resuelto del
# Anexo C de ISO 717-2, el forjado pesado desnudo de la Tabla C.1; el CI del
# título se lee sobre la suma de 100-2500 Hz que pide A.2.1).
l_n = np.array([62.1, 63.2, 63.5, 66.2, 68.5, 70.0, 71.7, 73.1,
                73.8, 73.5, 73.8, 73.3, 73.1, 73.0, 72.4, 71.2])
# Índice de impactos ponderado + término de adaptación espectral CI (ISO 717-2)
res_imp = building.weighted_impact_rating(l_n)
...
# Aquí la entrada fue Ln, así que la magnitud valorada es Ln,w:
fig, ax = plt.subplots()
...
Pasar este espectro a `weighted_impact_rating` reproduce el índice del Anexo C
de ISO 717-2 (tercios $L_\mathrm{n,w} = 79$; octavas 54, $C_\mathrm{I} = 0$) y,
leyendo $C_\mathrm{I}$ sobre la suma de 100–2500 Hz que fija A.2.1,
$C_\mathrm{I} = -11$ en tercios. La edición de 2020 de la Tabla C.1 suma en esa
celda las dieciséis bandas y da $-10$; la entrada del registro está en
...
| `temperature_c`, `relative_humidity_percent` | `float` | Clima representativo único: temperatura del aire (°C, cualquier signo), humedad relativa (0–100 %) |
| `static_pressure_kpa` | `float > 0` | Presión (estática) ambiente durante el ensayo (kPa) |
| `source_temperature_c`, `source_relative_humidity_percent`, `receiving_temperature_c`, `receiving_relative_humidity_percent` | `float` | Clima por recinto cuando el recinto emisor y el receptor se informan por separado (mismos rangos que arriba) |
