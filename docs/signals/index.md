...
61672-3 periodic tests it receives in service, and the IEC 60942 tests of the
calibrator itself, are not run (a laboratory's calibrator results are graded by
`metrology.verify_sound_calibrator`, which measures nothing), so a class
verdict here describes the algorithm and not a built device; [Compliance and
