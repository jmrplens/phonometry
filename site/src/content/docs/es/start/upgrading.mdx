...
| un `name` o un `source` vacíos | `CatalogueError` |
...
| `bounded_above` o `bounded_below` sobre un campo sin intervalo; un intervalo con un extremo que no es finito, o con el extremo bajo por encima del alto; una `uncertainty` negativa | `CatalogueError` |
...
| una densidad, una masa, una velocidad, una presión o un módulo, una resistividad al flujo, una rigidez por unidad de superficie, una longitud o un área negativas; una porosidad fuera de 0 a 1; una fracción de un todo fuera de 0 a 100 por ciento | `CatalogueError`; una temperatura en grados Celsius, una tasa de decaimiento por metro y un nivel en decibelios siguen pudiendo ser negativos |
...
| un campo de una subclase anotado con un tipo para el que el contrato no tiene comprobación | `TypeError` la primera vez que se construye la clase; un campo es un número o un entero que pueden ser `None`, un `bool`, un `str`, un `frozenset[str]` o un `Mapping[str, ...]` de ellos |
...
| `GroundSurface.porosity` con 36,5 en seis filas de la Tabla 6.7 de Cox y D'Antonio | `porosity_percent`, con las mismas cifras; `porosity` queda vacío en esas seis |

Ninguna fila publicada cambia de valor: todas las filas empaquetadas cumplían
ya el contrato salvo esas seis porosidades, que Cox imprime en tanto por ciento
en una columna de fracciones, y todas las filas porosas publicadas con los dos
módulos imprimen su factor de pérdidas.
...
puso la unidad en la etiqueta; no recalculó nada.
