...
distinto por medición como `m1`/`m2` (con `temperature1_c`/`temperature2_c`);
`measure_sound_absorption` supone un único clima. La $m$ por banda sale de una
atenuación $\alpha$ de ISO 9613-1 en dB/m
(`environment.air_attenuation(freqs, temperature_c=21.4,
relative_humidity_percent=54.0, atmospheric_pressure_kpa=101.325)`) convertida
con
...
`test_date`, `static_pressure_kpa`, `measurement_standard`, `laboratory`,
`operator`, `report_id` y `notes`. El campo `requirement` se ignora (la ISO 354
no tiene veredicto). La generación necesita reportlab y, para la figura que
incrusta la ficha, matplotlib (`pip install "phonometry[report,plot]"`); solo
se admite `engine="reportlab"`. La ficha se genera en inglés por defecto; pasa
...
| `temperature_c` / `temperature1_c`, `temperature2_c` | float | °C | por defecto `20.0`, 15–30 | Fija $c$ vía Ec. (6); `temperature2_c` por defecto es `temperature1_c` |
...
- [Catálogos publicados](/phonometry/es/reference/catalogues/#coeficientes-de-absorción): los
  coeficientes de absorción banda a banda que imprimen los libros, con el
  montaje sobre el que se midió cada uno.
