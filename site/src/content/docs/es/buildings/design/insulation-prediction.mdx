...
la fórmula (15) para la trayectoria directa
($L_\mathrm{n,d,w} = L_\mathrm{n,eq,0,w} - \Delta L_\mathrm{w} - \Delta L_\mathrm{d,w}$) y, para cada trayectoria por flancos,
la fórmula (16)
($L_{\mathrm{n},ij,\mathrm{w}} = L_\mathrm{n,eq,0,w} - \Delta L_\mathrm{w} + (R_{i,\mathrm{w}} - R_{j,\mathrm{w}})/2 - \Delta R_{j,\mathrm{w}} -
K_{ij} - 10\log_{10}(S_i/l_0 l_{ij})$), de modo que **la corrección de
...
- [Catálogos publicados](/phonometry/es/reference/catalogues/#aislamiento-acústico): la
  pérdida por transmisión banda a banda y los índices globales que imprimen
  los libros para construcciones reales.
