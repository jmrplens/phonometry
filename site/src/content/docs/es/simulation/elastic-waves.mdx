...
onda del agua sigue teniendo que avanzar a los 5958 m/s del acero,
...
Cada uno sale de una página, la Tabla C.1 de Bies 5e: la densidad es la que
imprime y las dos velocidades se siguen del módulo y del coeficiente de
Poisson que imprime al lado, por las formas cerradas que la propia tabla
imprime al final. Son valores representativos para documentación y
validación, que es lo que el libro dice de ellos, no datos certificados, así
que una probeta real hay que seguir introduciéndola a mano. La sección 1
importa `ALUMINIUM` en vez de escribir sus números, porque una copia aquí
quedaría libre de alejarse de la página. El constructor impone una sola cota,
...
### Un sólido de un catálogo

La fila de catálogo de un sólido entra allí donde entra un `Material`, sea de
`solids.PUBLISHED_SOLIDS` o de [tus propios catálogos](/phonometry/es/io/material-catalogues/):
el FDTD elástico lee de la fila la velocidad longitudinal de un sólido
extenso, la velocidad transversal y la densidad, y nunca la velocidad de placa o de barra
que una tabla imprime a su lado. El acero dulce de la Tabla C.1 de Bies es la
fila de la que sale `STEEL`, y el acero de Long, una velocidad impresa sin
decir cuál de las tres es, se rechaza en lugar de adivinarse:
...
print(round(mild_steel.plate_longitudinal_speed_m_s, 1))   # 5383.1, aquí nunca se lee
