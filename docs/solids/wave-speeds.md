...
`PUBLISHED_SOLIDS` holds forty rows read from two published pages: Hopkins
**Table A2**, twenty-five building materials, and Cremer 3e **Table 4.3**,
thirteen metals over fifteen rows. The key names the table, because both books
print a steel and they are not the same steel.
...
speeds whichever one its page printed, so the comparison is between two books
rather than between two waves. Cremer's own column is the bar speed: his
Eq. (3.32) defines `c_LII` as `sqrt(E/rho)`, and the Roman subscript counts the
directions along which the cross-sectional contraction is unconstrained, so
`II` is a rod.
