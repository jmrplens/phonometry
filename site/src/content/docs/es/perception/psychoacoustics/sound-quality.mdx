...
ax0.plot(tin.times, tin.tonality_vs_time,
         label=f"Tono en ruido (T = {tin.tonality:.2f} tu_HMS)")   # 1,19
ax0.plot(pn.times, pn.tonality_vs_time,
...
ax1.plot(res.times, res.roughness_vs_time)
...
($F'(z)$, 53 bandas), `bark`, `centre_frequencies`, `times`,
...
La DIN 45692:2009-08 mediante `sharpness_din()`: la ponderación del apartado 6
