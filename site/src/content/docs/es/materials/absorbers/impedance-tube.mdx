...
```python
for spacing, diameter_m in ((0.100, 0.100), (0.050, 0.100), (0.020, 0.029)):
    iso = materials.plane_wave_frequency_range(spacing, 343.2, diameter_m=diameter_m)
    astm = materials.plane_wave_frequency_range_astm(spacing, 343.2, diameter_m=diameter_m)
    print(f"tubo de {diameter_m * 1e3:.0f} mm, s = {spacing * 1e3:.0f} mm:",
...
que se le dio su geometría (`spacing`, `x1`, `diameter_m`) redibuja su propio
...
la longitud de onda (6.2.3). Pasar `diameter_m=` (y `shape=`) a
...
`measurement_standard`, `laboratory`, `operator`, `report_id` y `notes`
(`tube_diameter_m` y `mic_spacing` se dan en metros y se imprimen en milímetros).
El campo `requirement` se ignora (la ISO 10534-2 no tiene veredicto). La
...
fiche.report(
    ...
    metadata=ReportMetadata(
        ...
        tube_diameter_m=diameter_m,        # m (se imprime como 100 mm)
...
# TL(f) con la absorción con respaldo rígido superpuesta
...
tm.plot(frequencies=f, characteristic_impedance=407.0, language="es")
plt.show()
