...
def reflection_coefficient(
    ...
    *,
    ...
    :param grazing_angle_deg: Grazing angle(s) :math:`\varphi` from the
    ...
    """
    phi = np.atleast_1d(np.asarray(grazing_angle_deg, dtype=np.float64))
    if phi.size == 0 or not np.all(np.isfinite(phi)):
        msg = "'grazing_angle_deg' must be finite and non-empty."
        raise ValueError(msg)
    if np.any(phi < 0.0) or np.any(phi > _NORMAL_INCIDENCE_DEG):
        msg = "'grazing_angle_deg' must be within [0, 90] degrees."
        raise ValueError(msg)
...
class BottomLossResult:
    ...
    :ivar grazing_angle_deg: Grazing angles, in degrees.
    ...
    :ivar critical_angle_deg: The critical grazing angle, in degrees, or ``None`` if
...
class SeabedReflection:
    ...
    :ivar grazing_angle_deg: Grazing angles :math:`\varphi` from the interface,
        in degrees.
    ...
    def __post_init__(self) -> None:
        ...
        mismatched ``grazing_angle_deg`` it stops with matplotlib's ``x and y must
...
def seabed_reflection(
    ...
    *,
    ...
    Evaluates :func:`reflection_coefficient` at ``grazing_angle_deg`` for the given
