...
`OutdoorAttenuation.report(path)` needs the composed downwind level, so pass a
`sound_power_level` to `outdoor_propagation_attenuation` first. Its boxed number
is the A-weighted downwind level $L_{AT}(\text{DW})$ at the receiver; a limit
level supplied through the metadata `requirement` adds a PASS/FAIL verdict (a
lower level is better). Pass `language="es"` for the Spanish fiche.
