...
resistencia específica al flujo de aire $R_\mathrm{s}$ está en **Pa·s/m** (no
Pa·s/m²); la resistividad al flujo de aire $\sigma$ es $R_\mathrm{s}$ por metro
de espesor. Dos funciones auxiliares recorren esa cadena desde lo que muestra
realmente un instrumento: `linear_airflow_velocity(q_v, area)` es la definición
...
area)` es el $R_\mathrm{s} = R\,A$ del apartado 3.2, que además admite la otra
vía, `specific_airflow_resistance(pressure_drop_pa=…, velocity=…)`, para un
banco que registra $\Delta p$ y $u$ y nunca llega a formar $R$.
...
print(round(materials.specific_airflow_resistance(pressure_drop_pa=8.1, velocity=0.5e-3)))
```
...
mostrado en milímetros), `test_room`, `test_date`, `temperature_c`,
