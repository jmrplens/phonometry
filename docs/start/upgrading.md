...
| `TransferMatrix.plot(f, rho_c, ax)` | `.plot(ax, frequency=f, characteristic_impedance=rho_c)` |
...
What each `.plot()` draws and each `.report()` prints is untouched, the
`Signal` contract too, and so is every formula. 4.0 rearranged the furniture
and put the unit on the label; it did not recompute anything.
