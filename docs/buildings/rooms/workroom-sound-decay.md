...
The margin over the background is a judgement the library will make for you,
one octave band at a time, and it says which of the three cases each position
falls in rather than correcting anything. A region holding exactly the two
positions a regression needs, and a line read outside the distances it was
fitted over, both emit `SpatialDecayWarning`.
