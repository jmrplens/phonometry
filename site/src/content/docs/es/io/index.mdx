...
ninguna de un fabricante, así que una ficha técnica, una declaración de
prestaciones o un informe de ensayo propios van en un archivo JSON suyo, con
la procedencia del documento: su tipo, su versión, el día en que se
consultó, el laboratorio y el informe. `read_catalogue` lee ese archivo en
filas de las mismas clases que entregan todos los catálogos publicados, de
modo que el núcleo poroso de su ficha entra en `medium()` igual que uno de
un libro, y una cifra que la ficha imprime en kPa·s/m² o en centímetros se
convierte a partir de sus dígitos y queda anotada tal como la imprimió. Todos
los problemas de un archivo se informan a la vez, cada uno en su lugar del
archivo, y nada de lo que el archivo nombra se importa nunca.
...
<ScopeClaim status="not-covered">
**La biblioteca no trae datos de fabricantes.** Una ficha técnica se revisa
sin cambiar de título, sus condiciones rara vez permiten copiarla, y el valor
que necesita un proyecto es el de la ficha con la que se especificó, así que
sus catálogos se quedan en sus archivos y en los objetos que devuelve
`read_catalogue`: la biblioteca no guarda ninguna copia ni abre ningún
