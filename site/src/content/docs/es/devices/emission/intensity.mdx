...
print(res.frequencies, res.intensity_level)                  # por banda
...
fig, ax = plt.subplots()
ax.plot(fi.frequencies, fi.f2, "o-", label="F2 (presión-intensidad superficial)")
ax.plot(fi.frequencies, fi.f3, "s--", label="F3 (potencia parcial negativa)")
...
entra más potencia por ese trozo de la que sale, lo más normal en la cara a la
sombra de una máquina que está junto a otra más ruidosa, y completamente carente
de sentido si los canales están cruzados.
...
`verify_intensity_class` compara un espectro de $\delta_{pI0}$ medido con
ambas
...
for dr in (0.006, 0.012, 0.050):
    ...
    res = emission.sound_intensity(x, p2, fs, spacing=dr, fraction=3,
                                   limits=[20.0, 12500.0])
    utiles = res.frequencies < c / (4.0 * dr)      # descartar las bandas saturadas
    axs.semilogx(res.frequencies[utiles],
                 -10.0 * np.log10(res.bias_correction[utiles]),
                 label=f"{dr * 1000:.0f} mm")
    axs.axvline(res.max_valid_frequency, linestyle=":")
    axi.semilogx(res.frequencies,
                 np.full(res.frequencies.shape, 10.0 * np.log10(dr / 0.025)))
...
| `density` | float | kg/m³ | por defecto `1.204` | Densidad del aire |
| `speed_of_sound` | float | m/s | por defecto `343.0` | Velocidad del sonido (estimaciones de sesgo/validez) |
...
(`verify_intensity_class`, `residual_index_limits`,
...
sonda-analizador, con el espaciador que va a montar, y aporta: el procedimiento
está en *Medir el índice residual*, más arriba, y lo que phonometry hace con el
número es clasificarlo frente a la tabla 2. La
