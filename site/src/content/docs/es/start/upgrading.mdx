...
| `row.is_derived(field)` en un valor que la página imprime en otra unidad (los °F y psi de la Tabla 14.1 de Ver & Beranek, los sabinios de la Tabla 7.1 de Long) | `field in row.converted`; `row.converted[field]` es la cifra y la unidad que imprime la página, como `("3e5", "psi")` |
| `row.is_derived(field)` en una celda que la página deja en blanco e imprime en otra fila (Tabla 8.7 de Ver & Beranek, Tabla 30 de ASHRAE, capítulo 32 de Harris) | `field in row.carried`; `row.carried[field]` nombra la fila |
...
barra obtenida de un módulo y una densidad. Un valor que la página imprime en
otra unidad sigue siendo el número de la página, y también lo es uno que la
página imprime una sola vez para un bloque de filas, así que ninguno de los
dos es ya derivado. `basis_of` responde con la entrada del propio campo, si no
