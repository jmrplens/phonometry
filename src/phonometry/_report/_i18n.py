...
_STRINGS: dict[str, str] = {
    ...
    "The attenuation A = A<sub>div</sub> + A<sub>atm</sub> + A<sub>gr</sub> + A<sub>bar</sub> (Eq. (4)); a negative A<sub>gr</sub> is a net gain from ground reflection. The downwind level L<sub>fT</sub>(DW) = L<sub>w</sub> + D<sub>c</sub> &#8722; A (Eq. (3)); L<sub>AT</sub>(DW) is its A-weighted total.": "La atenuación A = A<sub>div</sub> + A<sub>atm</sub> + A<sub>gr</sub> + A<sub>bar</sub> (Ec. (4)); un A<sub>gr</sub> negativo es una ganancia neta por reflexión en el suelo. El nivel a favor del viento L<sub>fT</sub>(DW) = L<sub>w</sub> + D<sub>c</sub> &#8722; A (Ec. (3)); L<sub>AT</sub>(DW) es su total con ponderación A.",
    "A<sub>misc</sub> (foliage, industrial sites, housing; Annex A), reflections from vertical surfaces (clause 7.5) and the meteorological correction C<sub>met</sub> for the long-term average level (clause 8) are not included. The method is accurate to within about 1 dB to 3 dB for broadband noise up to 1000 m (Table 5).": "A<sub>misc</sub> (follaje, zonas industriales, viviendas; anexo A), las reflexiones en superficies verticales (apartado 7.5) y la corrección meteorológica C<sub>met</sub> para el nivel medio a largo plazo (apartado 8) no se incluyen. El método tiene una exactitud de aproximadamente 1 dB a 3 dB para ruido de banda ancha hasta 1000 m (Tabla 5).",
