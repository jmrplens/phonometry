...
different material. Across these six tables a named material's density agrees
to within 2,6 per cent, every time, while the moduli disagree by nineteen per
cent on aluminium and thirty on lead without anybody being wrong. So a density
past 8 per cent apart is a transcription error, and a modulus twice its
neighbour is the literature.
...
errata to register, and the accepted modulus of tin is near Bies' figure rather
than the one the other two share. Two books agreeing is not evidence, which is
why the catalogue keeps all six and nothing in it votes by majority.
