...
| `resample` | If True, resampling is performed: each band is filtered on a decimated sample rate (default True). A decimated `butter` octave bank sums its adjacent outputs to about +0.94 dB, past the +0.8 dB of class 1 in IEC 61260-1:2014 5.16, and is class 2; filtered at the full rate (`False`) it is class 1 on every requirement, and a one-third-octave bank is class 1 either way. |
