...
is the cheapest check there is on a row. Every quantity here was converted from
the degrees Fahrenheit and the pounds per square inch the page prints, so every
cell is marked as worked out rather than read and says what it was worked out
from. The page says in a footnote that the values were read off published
...
than named: about a dozen of them begin `Igual que 1 salvo que`, and the row
they refer to is one lookup away. Every dimension is printed twice, in
centimetres and in inches, and nineteen of the three hundred and twenty pairs
