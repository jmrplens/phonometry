...
Los $m'$ y $f_\mathrm{c}$ del elemento salen de su material y de su
espesor, $m' = \rho t$ y $f_\mathrm{c} = c_\mathrm{o}^2 / (1{,}8\, c_\mathrm{L} t)$
con la densidad y la velocidad de fase cuasilongitudinal de la tabla B.3, y
`HomogeneousElement.from_solid` calcula los dos a partir de la fila de
catálogo de un sólido, publicada o de [tus propios catálogos](/phonometry/es/io/material-catalogues/).
El factor de pérdidas interno nunca se lee de la fila, que puede guardar
cuatro, así que aquí se da como lo da la especificación del elemento. La
$f_\mathrm{c}$ sin redondear es la que, según la nota de la norma, usó su
cálculo:
...
print(np.round(from_row.sound_reduction_index[[0, 10]], 1))   # [31.8 54.9]  tabla L.3
```
