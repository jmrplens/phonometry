...
electrically. The setup differs mainly in where the signal enters.
...
measured at that same position with the source switched off. This is why the
...
band-by-band difference. Alternatively, when the absolute levels matter because
auditory masking and the reception threshold are in play, pass `levels=` and
`ambient=` instead so the level-dependent stages of the standard are applied
rather than a pure ratio.
...
for certifying a transmission channel. Write a project requirement as a numeric
STI, and use the letter to report it.
...
space*), the value a given location has about an 84 % probability of
...
position, or the mean minus one standard deviation (as those are three
different verdicts against the same limit). The 0.04-wide Annex F bands are
...
threshold corrections of Tables A.2 and A.3 (`levels=` and `ambient=`), the
