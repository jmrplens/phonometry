...
references:
  ...
  - type: standard
    ...
    year: 2012
    title: "Road traffic noise reducing devices — Test method for determining the acoustic performance — Part 1: Intrinsic characteristics of sound absorption under diffuse sound field conditions"
    designation: "EN 1793-1:2012"
    note: "Apartado 5, el índice global DLalpha y su límite de 0,99 en el cociente; apartado 6.1, la declaración al entero más próximo; anexo A, las categorías A1 a A5. Leída de la edición BS EN 1793-1:2012."
  - type: standard
    organization: "European Committee for Standardization"
    year: 2012
    title: "Road traffic noise reducing devices — Test method for determining the acoustic performance — Part 2: Intrinsic characteristics of airborne sound insulation under diffuse sound field conditions"
    designation: "EN 1793-2:2012"
    note: "Apartado 5.2, el índice global DLR; apartado 7.1, la declaración al entero más próximo; anexo A, las categorías B1 a B4. Leída de la edición BS EN 1793-2:2012."
  - type: standard
    organization: "European Committee for Standardization"
    year: 1997
    title: "Road traffic noise reducing devices — Test method for determining the acoustic performance — Part 3: Normalized traffic noise spectrum"
    designation: "EN 1793-3:1997"
    note: "Tabla 1, los dieciocho niveles relativos ponderados A por tercio de octava con los que se ponderan los dos índices globales. Leída de la edición BS EN 1793-3:1998."
---
...
### Un suelo de un catálogo

Una fila de `environment.PUBLISHED_GROUND`, o de [un catálogo propio](/phonometry/es/io/material-catalogues/),
da la impedancia por sí misma: `GroundSurface.medium` es el semiespacio
poroso de su resistividad al flujo efectiva, por el modelo de Delany y
Bazley o por el de Miki, que `ground_effect` recibe como `impedance`,
`barrier_insertion_loss` como `ground_impedance` y
`atmospheric_parabolic_equation` como `impedance`. La resistividad se lee a
través de `printed()`, así que una fila que imprime un intervalo se rechaza
en lugar de promediarse y, como el valor ajustado depende del modelo, una
fila que la página marca como ajustada con un modelo solo entra en ese
modelo. La clase D de Harmonoise, $\sigma$ = 200 kPa·s/m², da la curva de
Miki del caso anterior:
...
## 3. Lo que dice la declaración del producto (EN 1793)

Todo lo anterior es la barrera como física. Una barrera a la venta es un
producto, y lo que lleva su declaración son dos enteros, y ninguno de los dos
es una difracción: cuánto del sonido que le llega vuelve cruzando la carretera,
y cuánto la atraviesa. Los dos se ponderan con un espectro que nadie mide en
obra, el **espectro normalizado de ruido de tráfico** de la EN 1793-3:
dieciocho bandas de tercio de octava de 100 Hz a 5 kHz, con el máximo en 1 kHz
y doce decibelios menos en cada extremo.

<ThemeImage
  src="https://raw.githubusercontent.com/jmrplens/phonometry/main/.github/images/road_device_ratings.svg"
  alt="Tres paneles sobre las dieciocho bandas de tercio de octava de la EN 1793-3. A la izquierda, el espectro normalizado de ruido de tráfico de la EN 1793-3 en barras, de menos veinte decibelios en 100 Hz hasta un máximo de menos ocho decibelios en 1 kHz y de vuelta a menos dieciocho en 5 kHz, con una anotación que dice que el pico manda en el índice, y sobre ellas el espectro normalizado de ruido ferroviario de la EN 16272-3-1 como línea con marcadores, más bajo en 100 Hz y plano de 1,25 a 2,5 kHz donde el de carretera ya está cayendo. En el centro, el coeficiente de absorción de dos dispositivos: un casete absorbente que sube de 0,15 a 0,98 alrededor de 400 Hz y baja hasta 0,70, con un DL alfa de 8 decibelios en la categoría A3, y un panel de hormigón plano en 0,05 con 0 decibelios en la categoría A1. A la derecha, el índice de reducción sonora de dos dispositivos: un panel de hormigón que sube de 30 a 53 decibelios con un DL R de 42 decibelios en la categoría B4, y un casete metálico que sube de 21 a 34 decibelios con 29 decibelios en la categoría B3."
  width="100%"
/>

*La ponderación de la izquierda lo decide todo, y hay dos: a un dispositivo de
carretera se le juzga por lo que hace alrededor de 1 kHz, y a uno de ferrocarril
por lo que hace entre 1,25 y 2,5 kHz.*

La **EN 1793-1** valora la absorción como la energía que el dispositivo *no*
devuelve:
...
Un $\alpha_\mathrm{S}$ de cámara reverberante puede pasar de uno banda a banda,
lo que empujaría el cociente ponderado por encima de 1 y dejaría al logaritmo
sin argumento. El apartado 5 lo dice y lo arregla: el cociente se limita a
0,99, así que ningún dispositivo puede pasar de 20 dB por absorbente que salga
medido. `sound_absorption_rating` avisa cuando lo que ha respondido es ese
límite y no la medida.

La **EN 1793-2** valora el aislamiento acústico al ruido aéreo con la misma
ponderación, pero sobre la energía transmitida:
...
# Un casete con lana mineral tras una cara perforada, en las dieciocho bandas
# de tercio de octava de la EN 1793-3, de 100 Hz a 5 kHz.
...
Los dos se declaran redondeados al entero más próximo (EN 1793-1, apartado 6.1;
EN 1793-2, apartado 7.1), y cada uno tiene en su anexo A una escala normativa
que se lee sobre ese entero: de A1 a A5 la absorción, de B1 a B4 el
aislamiento, con A0 y B0 reservados para «sin determinar». Las escalas no
tienen huecos entre escalones porque el número que clasifican ya es un entero.

No son comparables y una declaración lleva los dos. Un panel de hormigón
liso devuelve casi todo al otro lado de la carretera y aun así es la mejor
pared; un casete absorbente que mantiene baja la reflexión puede ser la peor.
Cuál de los dos importa es una pregunta sobre el emplazamiento y no sobre el
producto: la absorción para una carretera en trinchera o entre pantallas
paralelas, el aislamiento para lo que haya justo detrás.

### Los mismos dos números junto a una vía

Una pantalla junto a una vía se valora igual y contra otro ruido. La
**EN 16272-3-1:2012** imprime el espectro normalizado de ruido ferroviario
sobre las mismas dieciocho bandas, y sus capítulos 5 y 6 son las dos fórmulas
de arriba con esa tabla en los pesos. El ruido de rodadura está más arriba que
el de una carretera: el espectro ferroviario es plano dentro de un decibelio
de 1,25 kHz a 2,5 kHz, donde el de carretera ya ha empezado a caer, así que un
absorbente que mejora con la frecuencia vale más junto a una vía que junto a
una carretera.

```python
from phonometry import environment

# Un absorbente poroso grueso, que sigue mejorando donde más suena la rodadura.
...
Las partes ferroviarias no imprimen escala de categorías: sus anexos son notas
de orientación sobre el uso del índice, así que un índice ferroviario es el
número y nada más, y el resultado lo dice llevando `None` donde un dispositivo
de carretera lleva su letra. El capítulo 6 trae además un desliz que conviene
conocer, porque cambia lo que uno se pone a buscar: dice que los índices de
reducción sonora se ponderan con «the normalised traffic noise spectrum
defined in Table 1», cuando la tabla 1 de esa norma es el espectro ferroviario
y la lista de símbolos bajo la fórmula dice «railway». Es la redacción de
carretera de la EN 1793-2, que se quedó ahí al copiar el capítulo.
...
- [Catálogos publicados](/phonometry/es/reference/catalogues/#superficies-de-suelo): la
  resistividad al flujo y los parámetros de modelo que imprimen los libros
  para hierba, nieve, asfalto y el resto de superficies de suelo.
