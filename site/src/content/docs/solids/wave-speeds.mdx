...
solve. `PUBLISHED_SOLIDS` holds forty rows read from two published pages, keyed
...
Both tables print a steel, at the same density, and they are not the same
steel: Hopkins gives a plate speed of 5 270 m/s and a Poisson ratio of 0,28,
Cremer a modulus of 210 GPa and 0,31. A flat `"steel"` would have to pick one
of them without saying so, and picking silently between two published values is
...
Every row carries all three longitudinal speeds whichever one its page
printed, so the comparison is between two books and not between two waves.
...
plate. He puts the gap at 16 per cent for `nu = 0,3`, which is what the three
functions above reproduce.
