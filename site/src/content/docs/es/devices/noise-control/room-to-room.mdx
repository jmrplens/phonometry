...
references:
  ...
  - type: book
    ...
    note: "El campo directo de la partición en el recinto receptor (apartado 7.5, Ecs. 7-71 y 7-72) y el Ejemplo 7-6, un operario a 1,5 m del muro, que reproduce receiver_distance_m."
  - type: standard
...
[Índices globales de aislamiento (ISO 717)](/phonometry/es/buildings/insulation/insulation-ratings/)). $S_\mathrm{w}$ es el área de la partición y $S_2 \alpha_2$ el área de absorción sonora
...
superficie radiante suma unos decibelios (el apartado siguiente les pone cifra). Lo mismo vale al revés en el lado emisor: $L_{p1}$ es el nivel
...
posiciones de micrófono a media longitud de onda de toda superficie. Más allá
del campo directo de la propia partición, que viene a continuación, es también
la razón de que este cálculo no pueda responder dónde colocar el puesto de
trabajo dentro del recinto receptor; cuando esa es la pregunta, la herramienta
son los métodos de fuentes imagen y de rayos de
[Fuentes imagen y campo estacionario](/phonometry/es/buildings/rooms/room-image-sources/).

### Un receptor junto a la partición

Barron, *Industrial Noise Control and Acoustics* (Marcel Dekker, 2003), apartado
7.5, lleva el campo directo de la partición al recinto receptor. Cerca del muro,
la potencia que radia se reparte sobre su cara; más lejos, el muro se comporta
como una fuente de directividad 2; y las dos descripciones se encuentran en
$r^* = (S_\mathrm{w}/2\pi)^{1/2}$. Sus ecuaciones (7-71) y (7-72), escritas con
las variables de la Ecuación (4.101), añaden un término dentro del logaritmo,
...
con $r_2$ la distancia del receptor a la partición. Las dos ramas coinciden en
$r^*$ y la segunda se desvanece al crecer $r_2$, así que la ecuación de Norton es
el límite lejano de la pareja de Barron. `receiver_distance_m` activa el término.
El Ejemplo 7-6 de Barron es una fábrica de papel: un refinador Jordan con un
nivel de potencia acústica de 105 dB está en un recinto de 900 m² de superficie
con un coeficiente de absorción medio de 0,05, y un operario está a 1,5 m del
muro de 16 m², de 30 dB de pérdida por transmisión, en un recinto de 100 m² a
0,35. El refinador está a 4 m del muro con un factor de directividad de 2, lo que
mueve el nivel que Barron imprime a su lado y no el nivel reverberante que excita
el muro:
...
source = noise_control.SourceRoom(
    power_level=105.0,
    room_constant=room.room_constant(900.0, 0.05),   # 47.37 m2
    directivity=2.0,
)
receiving = room.room_constant(100.0, 0.35)          # 53.85 m2, la R2 de Barron

far = noise_control.room_to_room_transmission(
    [1000.0], 30.0, 16.0, receiving, source=source,
)
near = noise_control.room_to_room_transmission(
    [1000.0], 30.0, 16.0, receiving, source=source, receiver_distance_m=1.5,
)
print(np.round(far.received_level, 1))    # [59.]
print(np.round(near.received_level, 1))   # [61.6]
```

Barron imprime 61,7 dB, que son estos 61,6 dB más los 0,1 dB que suma a todo
nivel por la impedancia característica del aire; la biblioteca no los suma, como
tampoco lo hace `steady_state_spl`. Entre los dos números hay dos lecturas de su
página. Escribe el término reverberante del recinto receptor sobre su constante
de sala $R_2 = S_2\alpha_2/(1-\alpha_2)$ donde Norton escribe $S_2\alpha_2$, así
que sus 53,85 m² entran como absorción del receptor. Y su ejemplo es de una sola
banda, así que la frecuencia central que se da aquí sólo la etiqueta. Su $r^*$
es de 1,60 m, el operario está dentro y recibe el cuarto entero: 2,65 dB sobre el
nivel reverberante. A 3 m el exceso es de 0,9 dB y a 10 m de 0,1 dB, que es donde
la Ecuación (4.101) vuelve a ser la respuesta. `required_transmission_loss` lleva
el mismo término, así que un muro especificado con una distancia de receptor
queda especificado para la mesa a esa distancia.
...
Con `receiver_distance_m`, el campo directo de la partición en un receptor del
recinto receptor, ecuaciones (7-71) y (7-72) de Barron (2003), en la cadena y en
la pérdida por transmisión necesaria.
</ScopeClaim>
...
superficie distinta de la partición que media longitud de onda.
</ScopeClaim>
</Scope>
