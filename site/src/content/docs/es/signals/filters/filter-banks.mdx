...
references:
  ...
  - type: web
    ...
    note: "Las recetas de coeficientes biquad y la parametrización por Q / ancho de banda / pendiente de shelving detrás de ParametricEQ (sección 4)."
  - type: web
    authors: ["Smith, J. O."]
...
la función, empieza por el apartado 3. En qué se diferencian las cinco
arquitecturas es la
...
el filtro (los picos de retardo de grupo de la sección 5 son esa resonancia,
...
<ThemeImage src="https://raw.githubusercontent.com/jmrplens/phonometry/main/.github/images/pole_migration.svg" alt="Tres paneles para la banda de tercio de octava de 25 Hz de un banco a 48 kHz. A la izquierda, la constelación en el plano z de la banda diseñada a la frecuencia completa: todos los polos colapsados sobre z = 1, con un recuadro ampliado que muestra los seis polos repartidos en menos de una milésima del radio y la anotación 1 menos r igual a 8,7e-05. En el centro, esa misma banda tal como la realiza de verdad el banco, a 906 Hz tras diezmar por 53: los polos han salido de z = 1 y quedan justo dentro del círculo unidad en dos grupos compactos a unos 10 grados por encima y por debajo del eje real, con 1 menos r igual a 4,6e-03. A la derecha, el mayor radio de polo de todas las bandas del banco en ejes log-log: el diseño a una sola frecuencia sube de forma sostenida desde 4e-05 a 12,5 Hz hasta 5e-02 a 16 kHz, mientras que el banco multitasa se mantiene entre 3,5e-03 y 4,6e-03 de 12,5 Hz a 630 Hz y, desde 800 Hz, donde sus bandas corren a la frecuencia completa, sigue la curva del diseño a una sola frecuencia" width="94%" />
...
frecuencia diezmada los tiene separados y 53 veces más lejos. El panel derecho
es la decisión de diseño resumida en una curva: el banco multitasa mantiene
cada banda diezmada a la misma distancia del círculo unidad, sea cual sea su
frecuencia central, y las bandas que corren a la frecuencia completa, desde
800 Hz, quedan más o menos igual de lejos o más.*

<details>
<summary>Cómo se midió esta figura</summary>

Los dos bancos salen de la tabla de parámetros del apartado 3, uno con el
...
**4.62e-03** a la frecuencia diezmada que el banco usa realmente
(`bank.factor[idx]` vale 53, el mayor factor que mantiene la frecuencia de
Nyquist de procesado de la banda en al menos dieciséis veces su borde superior
de banda, así que la banda corre a 906 Hz). SciPy lanza `BadCoefficients` al
leer el diseño a la frecuencia completa, y no el diezmado: ese aviso es el
...
## 2. Qué devuelve una llamada

Toda llamada devuelve un `OctaveFilterResult`: los niveles de banda en
`levels`, las `frequencies` sobre las que se apoyan y, cuando la llamada las
pidió, las señales por banda en `bands`. Los campos tienen nombre porque la
alternativa era una tupla cuya longitud dependía de una palabra clave, y quien
leía `_, _, bands = ...` tenía que contar comas para averiguar qué eran los dos
primeros.
...
# 31 83.0
ax = result.plot()            # el espectro de abajo
```

<ThemeImage src="https://raw.githubusercontent.com/jmrplens/phonometry/main/.github/images/octave_band_levels.svg" alt="Espectro de tercio de octava dibujado por el propio objeto de resultado: nivel de banda en decibelios frente a la frecuencia central de banda en eje logarítmico de 16 hercios a 20 kilohercios, con un punto marcado por banda. Un tono de 1 kilohercio se sitúa en 83 decibelios, unos cuarenta decibelios por encima de un suelo de ruido que ronda los 43 decibelios en la parte baja del rango y cae sostenidamente hasta 17 decibelios en la parte alta" width="88%" />

*El espectro de tercio de octava de un tono de 1 kHz sobre ruido. Cualquier
resultado de la biblioteca se redibuja así: el `result.plot()` de arriba produjo
exactamente esta figura.*

Hay dos accesores para los campos que una llamada puede no haber rellenado.
`require_levels()` y `require_bands()` devuelven el valor o dicen qué habría
tenido que pedir la llamada, lo cual es mejor que encontrarse un `None` varias
líneas más adelante:

```python
bands = filters.octave_filter(x, fs, sigbands=True).require_bands()
print(len(bands))
# 11

try:
    filters.octave_filter(x, fs).require_bands()
except ValueError as error:
    print(error)
# this result kept no band waveforms: call with sigbands=True to have them
```
...
| `zero_phase` | solo `.filter()` | bool | — | por defecto `False` | Filtrado adelante-atrás (offline); ver el apartado 6 |
...
`OctaveFilterBank.filter()`, mientras que llamar a `octave_filter(x, fs, zero_phase=True)` lanza
`TypeError`. El estado de streaming queda fijado al construir el
banco.
...
aceptación de la Tabla 1 de IEC 61260-1, el ancho de banda efectivo y la suma de
las salidas adyacentes, e informa de la clase (`1`, `2` o `None` si queda fuera
de ambas) con los márgenes por banda. Los bancos de octavas y de tercios de
octava por defecto son clase 1 en los tres, diezmados o filtrados a la
frecuencia completa; Verificación de clase de filtros explica por qué el
diezmado apenas mueve la suma.
...
6, 8 y 10 pasan todos la máscara de la Tabla 1 con un margen de clase 1 de
exactamente +0,40 dB.
...
profundidad en la banda atenuada, pero ni un decibelio de margen en la máscara.
Lo que manda a partir de ahí es la suma de las salidas adyacentes, que
IEC 61260-1:2014 también califica (5.16): el orden 4 se queda a 0,002 dB de la
clase 1 en ella y es clase 2 en conjunto, mientras que los órdenes 6, 8 y 10 la
cumplen con 0,04 dB, 0,08 dB y 0,07 dB. Y un orden mayor cuesta: cada par de polos de más alarga la resonancia (los picos de retardo de
grupo del apartado 5 crecen con el orden), que es la razón por la que el trabajo
con transitorios y los espectrogramas de ventana corta prefieren órdenes bajos.
La aritmética del margen en sí es
[Verificación de clase de filtros](/phonometry/es/signals/filters/filter-compliance/).

## 4. EQ paramétrico (`ParametricEQ`)
...
## 5. Señales de banda y retardo de grupo
...
impulse[0] = 1.0
imp_butter = bank_b.filter(impulse, sigbands=True).bands
imp_cheby2 = bank_c.filter(impulse, sigbands=True).bands
...
estrecha, reduciendo el nivel de banda ancha medido en unos 0,4 dB por banda
(filtrar dos veces eleva el módulo al cuadrado, y una banda Butterworth de
orden 6 deja pasar entonces 11/12 de su ancho de banda de ruido, −0,38 dB; un
tono puro dentro de banda no se ve afectado); prefiere el filtrado hacia
