...
$\theta + 273\ ^\circ\text{C}$, neither of which is the accurate figure.
Together they put the density 0,069 % high at 20 °C, and the library keeps
both as printed, because the error cancels: the same density appears in the
dynamic pressure of both test series, so the coefficient never sees it. That
...
That number belongs to the object rather than to the test point: a loss grows
as the square of the velocity and so does the head it is divided by, so
measuring the same object twice as fast gives the same coefficient.
...
measurement. Fewer than five and the library says so.
