...
## 3. Agradabilidad y eventualidad
...
encuestaron con el cuestionario inglés del Método A. Estas son las medianas por
...
la escala; `central_tendency="mean"` usa la media, que, al ser lineales las
fórmulas, es la media de las coordenadas de los encuestados.
...
el coeficiente de Pearson de datos normales y la aproximación habitual para el
de Spearman. `alternative="greater"` o `"less"` da el valor unilateral.

Conviene saber dos detalles de la página. La Fórmula (A.3) se imprime como
$1 - 1\,\tfrac{6\sum d_i^2}{n(n^2-1)}$, con un factor 1 suelto que la
biblioteca lee como el resto de composición que es. Y la Fórmula (B.2), la
covarianza de Pearson para datos de intervalo, divide por $n$; las
desviaciones típicas de (B.1) se toman entonces también con $n$, que es lo que
hace de $r$ el coeficiente habitual. `rho.plot()` dibuja los pares ordenados
...
el tiempo, que imprime A.3 f) de ISO/TS 12913-2; $N_5/N_{95}$ es la razón de
...
y elemento (A.2), la agradabilidad y la eventualidad de las Fórmulas (A.1) y
