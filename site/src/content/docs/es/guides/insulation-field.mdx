...
<ThemeImage src="https://raw.githubusercontent.com/jmrplens/phonometry/main/.github/images/insulation_rating_es.svg" alt="Índice de reducción sonora medido en tercios de octava con la curva de referencia de ISO 717-1 desplazada y el índice ponderado resultante a 500 Hz" width="80%" />
...
### Informe del Anexo C de ISO 717 (`.report()`)

Ambos resultados de calificación generan en PDF una ficha del Anexo C de
ISO 717 mediante el método `report(path)`: el enunciado del número único, la
gráfica de la curva medida frente a la referencia desplazada (el propio
`.plot()` del resultado) y la tabla de evaluación C.1 con la suma de
desviaciones desfavorables al pie. `WeightedRatingResult.report()` etiqueta la
ficha aérea de ISO 717-1 (`Rw (C; Ctr)`, desviaciones donde la referencia
supera a la medida); `ImpactRatingResult.report()` etiqueta la ficha de
impactos de ISO 717-2 (`Ln,w (CI)`, con el signo contrario).
`SoundReductionResult.report()` es una utilidad que califica el `R(f)` predicho
y escribe su ficha en una sola llamada.
...
# Calificación aérea a partir de un espectro R de 16 bandas (ISO 717-1 Anexo C)
R = [20.4, 16.3, 17.7, 22.6, 22.4, 22.7, 24.8, 26.6,
     28.0, 30.5, 31.8, 32.5, 33.4, 33.0, 31.0, 25.5]
building.weighted_rating(R).report("Rw_ficha.pdf")          # Rw (C; Ctr)
