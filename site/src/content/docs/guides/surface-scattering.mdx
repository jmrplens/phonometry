...
references:
  - type: book
    ...
    note: "ISBN 978-1-4987-4099-9. The reference monograph on diffuser theory and design, by the authors behind the ISO 17497-2 diffusion-coefficient method: the scattering-versus-diffusion distinction, the measurement rigs and the design guidance this page condenses."
  - type: standard
...
<ThemeImage src="https://raw.githubusercontent.com/jmrplens/phonometry/main/.github/images/diffusion_polar.svg" alt="A polar plot of the reflected sound-pressure level of a diffusing surface over 37 receivers from -90 to 90 degrees, the response spread almost uniformly over angle, giving an autocorrelation diffusion coefficient d of about 0.82" width="72%" />
...
Collected across the one-third-octave bands, the directional diffusion
coefficient $d(f)$ forms a `DiffusionSpectrum`, whose `report(path)` renders a
one-page diffusion test report (ISO 17497-2, Clause 8.5): the per-band table of
$d$ beside the $d(f)$ band-axis curve, with the random-incidence value $d$
(Clause 8.4) boxed. `verbose=True` adds the normalised $d_n$ column.

```python
import numpy as np
from phonometry import materials

# Per-band directional coefficients d(f) collected into a spectrum (here from a
# closed-form example; in practice each d comes from directional_diffusion_coefficient
# on that band's polar response).
...
<ReportPreview
	...
	description="One-page directional diffusion fiche: a metadata header, the per-one-third-octave table of the diffusion coefficient beside the d(f) band-axis curve with the normalised d_n companion, and the boxed random-incidence diffusion coefficient over the tested frequency range."
	caption="Directional diffusion fiche (DiffusionSpectrum.report), the d(f) spectrum with the normalised d_n."
/>
