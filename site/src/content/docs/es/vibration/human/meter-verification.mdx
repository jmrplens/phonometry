...
exposición a vibraciones** (PVEM): el que un trabajador lleva puesto durante
la jornada y que tiene que registrar la exposición al menos 12 h sin que nadie
lo toque (5.1 y 5.12). Mide las mismas magnitudes a través de las mismas
ponderaciones, y la parte 2 no le escribe un segundo juego de tablas: vuelve a
imprimir las de la parte 1 y cambia solo lo que necesita cambiar un medidor
que se lleva sobre el cuerpo. La pregunta de esta sección es, por tanto, a
cuáles de los números anteriores está sujeto un PVEM y cuáles son suyos.
...
applicable for PVEM». La parte 2 no le pide a un medidor personal ningún valor
eficaz móvil, así que los tiempos de decaimiento de la sección 9 tampoco
tienen equivalente en ella. `PVEM_INDICATION_TOLERANCES_PERCENT` es esa tabla,
