...
A sound power determination ends as a *document*. Both the enveloping-surface
...
`notes`); the measurement surface area $S$ and the $K_1$/$K_2$ corrections come
from the result itself and are printed in the result box and the basis strip.
Supplying `requirement` adds a PASS/FAIL verdict against a declared A-weighted
sound-power limit (a sound-power emission is a quantity where less is better, so
the source passes at or below the limit). `verbose=True` adds the
energy-averaged level $L_p'$ and the $K_1$/$K_2$ correction columns to the
table, and `language="es"` renders the Spanish fiche with comma decimals.
