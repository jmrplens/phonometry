...
references:
  - type: book
    ...
    note: "ISBN 978-1-4987-4099-9. La monografía de referencia sobre teoría y diseño de difusores, de los autores del método del coeficiente de difusión de ISO 17497-2: la distinción dispersión-difusión, los montajes de medida y la guía de diseño que esta página condensa."
  - type: standard
...
La respuesta de un solo plano de abajo es real: una predicción en campo libre
por elementos de contorno (COMSOL) de un difusor de residuos cuadráticos
$N = 7$ y su panel de referencia plano a 1000 Hz, incidencia normal, muestreada
en el semicírculo estándar de 37 puntos (espaciado de 5°, de $-90°$ a $+90°$) en
el plano de máxima difusión, de J. M. Requena-Plens, *Estudio y caracterización
de difusores acústicos mediante el método de los elementos finitos* (trabajo fin
de máster, Universitat Politècnica de València, 2018). Los niveles polares son
$L_i = 20\,\lg|p_i|$ de la predicción de campo lejano publicada; los mismos
valores alimentan el conjunto de conformidad de phonometry.

```python
from phonometry import materials

# Niveles reflejados L_i (dB) en el semicírculo de 37 puntos de un QRD N = 7.
levels_qrd = [
...
# El panel de referencia plano en el mismo arco: un par de lóbulos especulares.
levels_flat = [
...
d = materials.directional_diffusion_coefficient(levels_qrd)    # fórmula (5)
...
# levels_qrd: los 37 niveles del QRD definidos en el ejemplo de arriba.
...
<ThemeImage src="https://raw.githubusercontent.com/jmrplens/phonometry/main/.github/images/diffusion_polar_es.svg" alt="Un diagrama polar del nivel de presión sonora reflejado de un difusor de residuos cuadráticos N=7 sobre 37 receptores de -90 a 90 grados, con la energía repartida por el semicírculo en varios lóbulos, dando un coeficiente de difusión por autocorrelación d de aproximadamente 0,76" width="72%" />

*El QRD reparte la energía reflejada por el semicírculo en varios lóbulos en
lugar de un único pico especular, de modo que el coeficiente de difusión por
autocorrelación es alto ($d \approx 0{,}76$). El panel de referencia plano
colapsa la respuesta en un par especular estrecho y baja a $d \approx 0{,}14$.*
...
# levels_qrd: los 37 niveles reflejados L_i del QRD N = 7 del ejemplo de arriba
# (campo lejano COMSOL, 1000 Hz, incidencia normal), en el semicírculo de -90 a
# 90 grados con espaciado de 5 grados. La energía se reparte en varios lóbulos,
# así que el coeficiente d de la fórmula (5) es alto.
angles = np.arange(-90.0, 90.5, 5.0)
result = materials.directional_diffusion(angles, levels_qrd)
