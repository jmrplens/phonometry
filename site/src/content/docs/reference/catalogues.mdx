...
The ground catalogue works the same way, and a row's `medium()` is the porous
half-space the outdoor models take as the ground, worked out from the
resistivity the page prints:
...
This page shows what the library has read from a book or a standard, and it
will never show a manufacturer's table: a data sheet is revised under the
same title, its terms rarely allow copying it, and the value a project needs
is the one on the sheet it was specified against. Your own data sheets,
declarations of performance, test reports and measurements go in a catalogue
file of yours, a JSON document or a spreadsheet saved as CSV, which
`io.read_catalogue` reads into the same row classes as
the tables below, and every lookup by name searches it with `catalogue=`,
alone or joined with a published catalogue.
[Your own catalogues](/phonometry/io/material-catalogues/) shows how, from a
published table written out as a template to the refusal that points at a
cell. A data sheet's practical absorption coefficients, a test report's
one-third-octave coefficients, a laboratory's sound reduction index and a
covering's impact improvement have row classes of their own that no table
below uses, and
[Reading a material datasheet](/phonometry/materials/reading-a-datasheet/)
says which number on a sheet is which. The JSON Schema of a catalogue file,
for every row class the library publishes, is served at
[`schemas/phonometry-catalogue-1.json`](https://jmrplens.github.io/phonometry/schemas/phonometry-catalogue-1.json),
so an editor completes a file of yours as you type it. A manufacturer's table
pasted into an issue or a pull request is never merged, and nothing on this
page comes from one.
...
These are the numbers the library's own plateau-method prediction reads, and
it takes a row of this table, or a row of your own, as its material. The
