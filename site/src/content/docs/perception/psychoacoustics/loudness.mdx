...
($N'(z)$, 240 bins of 0.1 Bark), and for time-varying runs `n5`, `n10`, `times`
...
plt.plot(tv.times, tv.loudness_vs_time, label="N(t)")
...
caricatures. `loudness_vs_time` and `times` carry the trace, so a record can be
