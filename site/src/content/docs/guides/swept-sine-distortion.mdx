...
ax.axhline(100.0 * (a2 / 2.0) / h1, ls=":", label="Chebyshev level a2/2")
ax.axhline(100.0 * (a3 / 4.0) / h1, ls=":", label="Chebyshev level a3/4")
...
- The phase utilities operate on any one-sided response: an `rfft` of a
  ...
  uniform grid, or a design target magnitude for equalization.
