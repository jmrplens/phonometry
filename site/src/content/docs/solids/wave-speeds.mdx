...
solve. `PUBLISHED_SOLIDS` holds seventy-eight rows read from three published pages, keyed
...
Mechel prints a modulus and no Poisson ratio, so his rows give the bar speed
and stop there. The plate and the bulk speed would need a ratio he does not
print, and a 0,3 would reproduce his table nicely and would be a number he did
not give.
...
What crosses between all three books with no assumption at all is `h f_c`, the
product of a plate's thickness and its critical frequency, which Hopkins prints
as a column and Mechel as `f_cr d`. It needs no Poisson ratio, no speed of
sound and no conversion, which makes it the cheapest cross-check there is
