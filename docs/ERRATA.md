...
- **The print:** the paragraph opens with "a normal distribution having a
  ...
  value was shifted by an amount, Δµ, equal to 1,04 dB [from Equation (22)]";
...
- **Evidence:** the paragraph read on PDF page 42 (printed p. 34), Equation
  ...
  and 44 (printed pp. 35 and 36), all of BS ISO 13474:2009, which reproduces
  ISO 13474:2009 (first edition, 2009-06-15). Every value was recomputed from
  Table A.3 on PDF page 40 (printed p. 32) of the same edition.
...
- **The problem:** Equation (24) defines the probability that the level
  ...
  so the rarer the level. All five printed values are reproduced to the
  printed digit by a cumulative curve formed from the lower end of the plotted
  axis rather than from minus infinity,
  $1 - \int_{15}^{x} \rho^{*}(x')\,\mathrm{d}x'$, with the 07:00 to 19:00
  column of Table A.3 as printed: 21,69; 31,49; 40,58; 43,16 and 47,97 dB.
  That curve counts the probability below 15 dB, 0,21 %, in every exceedance,
  which moves the levels at the smallest percentages most. Figures A.2 and A.3
  both start at 15 dB, and the curve of Figure A.3 starts there at exactly 1,
  where Equation (24) gives 0,998.
- **Evidence:** Figure A.3 on PDF page 44 (printed p. 36), Figure A.2 on PDF
  ...
  p. 13), all of BS ISO 13474:2009, which reproduces ISO 13474:2009 (first
  edition, 2009-06-15). The levels were recomputed from Table A.3 on PDF page
  40 (printed p. 32) of the same edition.
- **Library behaviour:** `SelDistribution.exceedance` evaluates Equation (24)
  ...
  printed 31,5 dB, and "ISO 13474:2009 Equation (24), Figure A.3" reproduces
  all five printed levels by the reading above, from the library's exceedance
  and the 15 dB of the figure.
- **Status:** unreported.
...
- **Evidence:** the paragraph as printed, read on the page. Verified on PDF
  ...
  $L_i$ under it.
- **Library behaviour:**
  [`airborne_insulation_rating`](../src/phonometry/environment/propagation/noise_reducing_devices.py)
...
- **Library behaviour:**
  [`temperature_field_correction_db`](../src/phonometry/noise_control/silencer_in_situ.py)
...
- **Library behaviour:**
  [`corrected_distribution_value`](../src/phonometry/room/spatial_decay.py)
  ...
  [`tests/room/test_spatial_decay.py`](../tests/room/test_spatial_decay.py)
...
- **Library behaviour:**
  [`DECADE_TO_DOUBLING`](../src/phonometry/room/spatial_decay.py) carries the
  printed 0,3, because the printed constant is what reproduces the printed
  results, and
  [`level_excess_at`](../src/phonometry/room/spatial_decay.py) divides by
...
- **Library behaviour:**
  [`NORMALIZED_OFFSET_DB`](../src/phonometry/room/spatial_decay.py) carries the
  ...
  [`spectrum_distribution_value`](../src/phonometry/room/spatial_decay.py)
  ...
  in [`tests/room/test_spatial_decay.py`](../tests/room/test_spatial_decay.py)
...
- **Library behaviour:**
  [`level_excess_at`](../src/phonometry/room/spatial_decay.py) implements
...
- **Library behaviour:**
  [`workstation_level_increase`](../src/phonometry/room/workroom_prediction.py)
...
- **Library behaviour:**
  [`PassByRegression`](../src/phonometry/environment/sources/statistical_pass_by.py)
...
- **Library behaviour:** the catalogue holds the density, the $f_{cr}d$ and
  ...
  [`tests/solids/test_catalogue.py`](../tests/solids/test_catalogue.py) pin
...
- **Library behaviour:** the catalogue holds the row and its molar mass, and
  ...
  [`phonometry.fluids.ideal_gas`](../src/phonometry/fluids/gas.py), which
  refuses a ratio at or below 1 whoever passes it, and
  `test_a_ratio_of_specific_heats_at_or_below_one_is_refused` in
  [`tests/fluids/test_gas.py`](../tests/fluids/test_gas.py) pins that.
- **Status:** not reported.
...
- **Library behaviour:** both rows are kept, with the description and the
  ...
  [`tests/materials/diffusers/test_scattering_catalogue.py`](../tests/materials/diffusers/test_scattering_catalogue.py)
...
- **Library behaviour:** the row keeps the width as printed, because a
  ...
  [`tests/materials/diffusers/test_diffusion_catalogue.py`](../tests/materials/diffusers/test_diffusion_catalogue.py)
...
- **What the library does:** the row is served under the name the page
  prints, in
  [`PUBLISHED_TRANSMISSION_LOSS`](../src/phonometry/building/catalogue.py),
...
- **Beranek & Mellow 2e Table 7.1, the Delany and Bazley column:** the table
  ...
  [`DELANY_BAZLEY_COEFFICIENTS`](../src/phonometry/materials/absorbers/porous.py)
