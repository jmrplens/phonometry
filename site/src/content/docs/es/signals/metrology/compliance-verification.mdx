...
Otras cinco funciones llevan el mismo prefijo `verify_` y responden a una
...
escribe las tolerancias. Cuatro devuelven un objeto de resultado cuyo `passes`
lo dice y cuyos demás campos dicen dónde y por cuánto;
`verify_running_rms_decay` califica una fila impresa y devuelve el `bool`.
...
- **Qué es un veredicto de medición.** Todavía más estrecho. A los cinco
...
`.plot()` y la ficha acreditada); los cinco verificadores que califican un
instrumento medido en vez de un diseño, y
dónde está la página de cada uno; cómo leer y citar el informe de conformidad
...
cumple o no cumple: `verify_aircraft_noise_system` frente a IEC 61265:1995, y
