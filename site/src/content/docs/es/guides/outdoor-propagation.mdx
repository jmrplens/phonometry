...
`OutdoorAttenuation.report(path)` necesita el nivel a favor del viento
compuesto, así que pasa un `sound_power_level` a
`outdoor_propagation_attenuation`. Su número destacado es el nivel a favor del
viento con ponderación A $L_{AT}(\text{DW})$ en el receptor; un nivel límite
indicado mediante el `requirement` de los metadatos añade un veredicto
CUMPLE/NO CUMPLE (un nivel más bajo es mejor). Usa `language="es"` para la ficha
en español.

```python
import numpy as np
from phonometry import ReportMetadata, Barrier, outdoor_propagation_attenuation
...
result.report(
    ...
    metadata=ReportMetadata(
        ...
        requirement=50.0,  # nivel maximo aceptable a favor del viento
    ),
    language="es",
)
```
...
result.report(
    ...
    metadata=ReportMetadata(
        ...
        requirement=8.0,  # perdida por insercion media minima requerida
    ),
    language="es",
)
```
