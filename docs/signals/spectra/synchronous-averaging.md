...
res = signals.time_synchronous_average(recording, fs, period_s=period_s, n_averages=n_avg)

fig, (ax0, ax1) = plt.subplots(1, 2, figsize=(11, 4.6))
t_ms = 1e3 * res.times
ax0.plot(t_ms, recording[:m], color="#cccccc", label="One noisy period_s")
...
ax1.plot(orders, signals.comb_filter_response(freqs, period_s, 32), color="#2ca02c",
         label="N = 32 (power of two)")
ax1.plot(orders, signals.comb_filter_response(freqs, period_s, 20), color="#1f77b4",
...
period_s = 1.0 / 32.0
signals.comb_filter_response(np.array([16.0 / period_s]), period_s, 8)   # 1.0 at a tooth
signals.comb_filter_response(np.array([0.25 / period_s]), period_s, 2)   # 1/sqrt(2)
signals.comb_filter_response(np.array([0.5 / period_s]), period_s, 2)    # 0.0 at a node
```
...
```python
res = signals.time_synchronous_average(recording, fs, period_s=period_s, n_averages=100)
...
true = (np.cos(2 * np.pi * m / period_s)
        + 0.5 * np.cos(2 * np.pi * 3 * m / period_s + 0.7)
        + 0.25 * np.cos(2 * np.pi * 5 * m / period_s + 1.1))
...
errors = []
for n in counts:
    res = signals.time_synchronous_average(recording[:n * samples], fs, period_s=period_s,
                                           n_averages=n)
...
leak_20 = signals.time_synchronous_average(recording, fs, period_s=period_s, n_averages=20)
leak_32 = signals.time_synchronous_average(recording, fs, period_s=period_s, n_averages=32)
...
```python
fs = 8192.0
period_s = 1.0 / 31.7                       # fs * period_s is not an integer
t = np.arange(int(40 * period_s * fs)) / fs
recording = np.cos(2.0 * np.pi * t / period_s)  # one cycle per revolution
res = signals.time_synchronous_average(recording, fs, period_s=period_s)
res.interpolated                          # True: fractional-delay alignment
res.samples_per_period                    # integer samples of one period_s
```
