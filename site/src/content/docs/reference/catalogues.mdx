...
converted and is marked as worked out rather than read, with the figure the
page prints beside it. The loss factor is the one column that was not: it has
...
is sixteen times stiffer along the grain than across it, and an instrument
...
instrument is made of, carry the four stiffnesses a thin orthotropic plate
actually has: along the grain, across it, in twisting, and the coupling
between the two. They are stiffnesses of a plate rather than moduli of a
material, so they do not compare with the fir and spruce in the solids table.
The page prints the woods as its columns and this table turns it so that a
wood is a row. Two of maple's stiffnesses are the author's own guesses, which
the page marks and the table shows, and maple's printed scaling factor does
not follow from its own row, which is registered in this project's errata.
...
the material brings, how deep the coincidence dip goes, and how wide the
plateau is in frequency. The first is a density in disguise, kilograms per
square metre per millimetre, which is the density divided by a thousand; it is
kept in that unit because that is the unit the method uses it in. None of the
three is a transmission loss. The measured insulation of real constructions is
in the transmission loss table below.
...
Forty-eight rows of measured impact insulation: forty-two floor-ceiling
constructions with the impact insulation class of each, and six elastic surface
treatments with the improvement each one adds. The class belongs to neither the
...
The two ratings are two quantities and no row carries both. A class is an
