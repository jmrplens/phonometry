...
recepción, pasa `levels=` y `ambient=` en su lugar para que se apliquen las etapas
...
- **Efectos dependientes del nivel.** El STI no es invariante con el nivel: el
  ...
  encima del $L_\mathrm{Aeq}$ del habla continua en la posición) y pasa `levels=` y
...
- **Ruido de fondo impulsivo y fluctuante.** Una herramienta que cae o un
  ...
  `levels=`/`ambient=`. Una comprobación rápida es ejecutar el analizador con
...
curve = [speech.sti_from_impulse_response(
    ir, fs, levels=shape - shape_total + t, ambient=ambient).sti for t in totals]
...
| `levels` | vector de 7, opcional | dB SPL | por defecto `None` | Activa el enmascaramiento auditivo + umbral de recepción (Tablas A.2/A.3) |
| `ambient` | vector de 7, opcional | dB SPL | requiere `levels` | Niveles de banda del ruido ambiente |
...
calculado sin `levels=`: ese resultado lleva como mucho el factor plano de
...
- **Los espectros de la propia medición son lo que quita el paso 2.** Un
  `measured_levels` o un `measured_ambient` equivocados sacan la corrección
...
| `measured_levels` | vector de 7 | dB SPL | obligatorio | Niveles de banda del habla durante la medición |
| `measured_ambient` | vector de 7, opcional | dB SPL | por defecto `None` | Niveles de banda del ruido de fondo durante la medición |
| `operational_levels` | vector de 7 | dB SPL | obligatorio | Niveles de banda del habla de la condición simulada |
...
Tablas A.2 y A.3 (`levels=` y `ambient=`), las letras de valoración del Anexo
