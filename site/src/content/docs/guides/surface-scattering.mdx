...
finite-element free-field simulation of an $N = 7$ quadratic-residue diffuser
and its flat reference panel at 1000 Hz, normal incidence, sampled on the
standard 37-point semicircle (5° spacing, $-90°$ to $+90°$) in the plane of
maximum diffusion, produced by the maintainer (J. M. Requena-Plens). The polar
levels are $L_i = 20\,\lg|p_i|$; the diffusion coefficient is cross-validated
against an independent MATLAB implementation of ISO 17497-2 Formula (5), and
the same values drive the phonometry conformance suite.
