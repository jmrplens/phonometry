...
En esa fórmula no hay término $K_1$ aparte porque $\overline{L_p}$
(sin prima, a diferencia del de la sección 1) es la media sobre la superficie de los
...
| `temperature_c` | float | °C | por defecto `23.0` | Fija $C_1$, $C_2$ |
| `static_pressure_kpa` | float | kPa | por defecto `101.325` | Fija $C_1$, $C_2$ |
...
print(round(corr.static_pressure_kpa, 1))    # 87.7 kPa, de la Ec. G.2
...
`integration_time`. `reference_atmosphere_correction(temperature_c,
static_pressure_kpa=…, altitude=…)` devuelve un
`ReferenceAtmosphereCorrection`
con `c1`, `c2`, su `total` y la `static_pressure_kpa` empleada.
