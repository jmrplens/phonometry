...
### Which reference value is each level counted from?

A level in decibels is a ratio to a reference value, so it means nothing
until the reference is named. ISO 1683:2015 fixes one per quantity, and the
library counts every level from it unless the standard a function implements
names another. The table is published as
`metrology.ISO1683_REFERENCE_VALUES`, keyed by medium and then by quantity:

| Quantity | Gases (Table 1) | Liquids (Table 2) | Vibration (Table 3) |
|:---|:---|:---|:---|
| Sound pressure | 20 µPa | 1 µPa | |
| Sound exposure | (20 µPa)² s | 1 µPa² s | |
| Sound power | 1 pW | 1 pW | |
| Sound energy | 1 pJ | 1 pJ | |
| Sound intensity | 1 pW/m² | 1 pW/m² | |
| Displacement | | 1 pm | 1 pm |
| Velocity | | 1 nm/s | 1 nm/s, or 50 nm/s (note b) |
| Acceleration | | 1 µm/s² | 1 µm/s² |
| Force | | | 1 µN |
| Distance, for compound quantities | | 1 m | |

The same pressure reads $10\lg(20^2/1^2) \approx 26.0$ dB higher against the
...
print(p0.value, p0.unit, p0.printed)  # 2e-05 Pa 20 µPa
```
...
- API reference: [`signals.levels`](https://jmrplens.github.io/phonometry/reference/api/signals/levels/), and [`metrology.reference_values`](https://jmrplens.github.io/phonometry/reference/api/metrology/reference-values/) for the ISO 1683 table.
...
(≡ LEP,d), anchored at 3.2 Pa²h ⇔ exactly 90 dB.
ISO 1683:2015, *Acoustics. Preferred reference values for acoustical and
vibratory levels*: Tables 1 to 3, the reference value every level here is
counted from, published as `metrology.ISO1683_REFERENCE_VALUES`, and note b of
Table 2, the 26.0 dB between the air and water pressure references. The ISO 1996-1/-2
