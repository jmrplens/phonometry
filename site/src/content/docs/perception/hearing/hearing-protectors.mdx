...
description: "The three ISO 4869-2 methods that turn a hearing protector's measured attenuation into the effective A-weighted level: the octave-band method, the HML values and the single number rating, all three set to one protection performance."
references:
  - type: standard
  ...
  - type: standard
    ...
    note: "Where the per-subject attenuation values come from: the real-ear attenuation at threshold measured on 16 subjects, which is the input every method here starts from."
---
