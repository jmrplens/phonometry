...
description: "A data sheet, a declaration of performance, a test report or a measurement of your own as a catalogue file: a published table as the template, the JSON document and its provenance, a spreadsheet saved as CSV, refusals that point at a cell, the rows in the porous models, a room model, the plateau method, a floating floor, the elastic simulation, the outdoor ground models, the ISO 12354 detailed model and every lookup by name, how to write a bound, a word or another unit, which row class holds which quantity, the JSON Schema an editor completes a file with, and what the library never does with your data."
references:
  - type: standard
    ...
    primary: true
    note: "The text a catalogue file is. The reader holds a file to it strictly: a NaN or an Infinity, which only some readers accept, and a name written twice in one object, whose meaning RFC 8259 leaves to the reader, are refused."
  - type: standard
    organization: "Internet Engineering Task Force"
    year: 2013
    ...
    note: "How every problem in a file is located: /rows/1/porosity is the porosity of the second row."
  - type: standard
    organization: "Internet Engineering Task Force"
    year: 2005
    ...
    note: "The CSV file a spreadsheet saves: one record per line ending in CRLF, the same number of fields on every line, and a field that holds a line break, a double quote or the delimiter enclosed in double quotes, with a quote inside it doubled. The reader takes a semicolon or a tab as the delimiter too, as the header declares."
  - type: standard
    ...
    note: "The practical sound absorption coefficient a European data sheet prints, rounded to steps of 0.05 and capped at 1.00: a rating of a reverberation-room measurement and not a Sabine coefficient, which is why it has no place in AbsorptionSpectrum."
  - type: standard
    organization: "International Organization for Standardization"
    year: 2010
  ...
  - type: standard
    ...
    note: "The dynamic stiffness s' of an installed layer and the apparent dynamic stiffness s't of a test specimen, two fields of ResilientLayer, and clause 8.2, which turns the second into the first with the lateral airflow resistivity."
  - type: standard
    organization: "International Organization for Standardization"
    year: 2017
    ...
    note: "The homogeneous element HomogeneousElement.from_solid builds from a solid's row: m' = ρt, and the critical frequency of Formula (B.2) with the quasi-longitudinal phase velocity of Table B.3, which is a row's plate speed. The internal walls of the worked building of Annex L are the example."
  - type: report
    ...
    note: "The 2020-12 dialect io.catalogue_schema writes the schema of a catalogue file in, and the published schema is written in."
  - type: report
    ...
    note: "The keywords the schema states the form of a catalogue file with: the types, the words a key may hold, the patterns of a name, the lengths of a text and the limits a field's unit sets."
---
...
[Choosing the ground impedance](/phonometry/environment/propagation/ground-barriers/#choosing-the-ground-impedance)
...
Reading a catalogue of your own from a JSON file or from a CSV file with its
...
A spreadsheet's own format (XLSX, ODS) is not read: save the sheet as
...
- [Files](/phonometry/io/): what the file layer of the library reads and writes, and why.
- [Published catalogues](/phonometry/reference/catalogues/): every table the library has read from a page, the same row classes with the book's cells.
- [Reading a material datasheet](/phonometry/materials/reading-a-datasheet/): what each number on a data sheet is, and which row class holds it.
- [Porous and Multilayer Absorbers](/phonometry/materials/absorbers/porous-absorbers/): the models a `PorousMaterial` row goes into.
- [Dynamic stiffness of resilient materials](/phonometry/materials/resilient/dynamic-stiffness/): $s'$, $s'_\mathrm{t}$ and clause 8.2 in full.
- [Predicting Panel Sound Insulation](/phonometry/buildings/design/panel-sound-insulation/): the plateau method a `PlateauMaterial` row draws.
- [Elastic waves and fluid-solid coupling](/phonometry/simulation/elastic-waves/): the solver a solid's row is a medium of.
- [Spherical ground effect and advanced barriers](/phonometry/environment/propagation/ground-barriers/): the models a ground row's `medium()` goes into.
- [Detailed Per-Band Prediction (ISO 12354)](/phonometry/buildings/design/detailed-prediction/): the ISO 12354 chain an element from `from_solid` enters.
