---
title: "Railway vibration by category of train (E DIN 4150-2)"
description: "What the draft of August 2023 changes in how a railway is judged for the people in a building: one clock interval per passage, a KB_Fmax that is 1,5 times the r.m.s. of a category, a weighting factor per kind of train and alignment, the night-time upper value of a new line, the 25 % rule for an altered one, and the rest of the draft's changes to DIN 4150-2, with an edition switch."
references:
  - type: standard
    ...
    primary: true
    note: "A draft, intended to replace DIN 4150-2:1999-06. Table 1 with its one changed cell, Formulae (5) to (8) and Table 2 of 6.5.3, the night-time upper value of 6.5.3.5, the 25 % rule of 6.5.3.6, the existing-road tolerance of 6.5.2, the induced seismic event of 6.5.1.3, and Table 3 as printed. Annex B, Examples 8 and 9, is the oracle of the conformance rows; Examples 1, 2 and 4 to 7 are those of the 1999 edition, and Example 3 is reworked without the 15 % argument."
  - type: standard
    organization: "Deutsches Institut für Normung"
    year: 1999
    title: "Erschütterungen im Bauwesen — Teil 2: Einwirkungen auf Menschen in Gebäuden"
    designation: "DIN 4150-2:1999-06"
    note: "The edition in force, which the library reads by default; the draft's changes are read with edition='2023'."
---
...
The [1999 edition of DIN 4150-2](/phonometry/vibration/immission/people-in-buildings/)
...
<ThemeImage src="https://raw.githubusercontent.com/jmrplens/phonometry/main/.github/images/diagram_railway_monitoring.svg" alt="A section through a railway and the building beside it, a strip of train passages, and the formulae. Top left, a tram runs on the surface and a metro in a tunnel below it, with arrows through the ground to the building. In the upper storey, the room to be protected, a transducer stands at the middle of the floor span with arrows for the vertical z and the horizontals x and y, marked 1. A second transducer sits on the foundation in the basement, marked 3, and a dashed arrow marked 2 rises from it to the floor of the room. Three boxes at the right explain the numbers: the floor point takes z, x and y where the vibration is expected to be strongest, for z mostly the middle of the span; the transfer to the room is a linear factor or a frequency-dependent transfer function, preferably measured, with its uncertainty only as an upper bound; the substitute point, at the foundation for monitoring over weeks or months, is held to DIN 45669-2. A line beneath says that from a substitute point only KB Fmax below A o and KB FTr below A r can be shown. In the middle, a time axis from 0 to 380 seconds carries four passages, a metro running north and one running south with small humps and a tram running east and one running west with large ones, each inside one dashed interval however long it lasts. Under them, per category: 14 passages giving 0.037 and 0.055, 14 giving 0.047 and 0.070, 9 giving 0.406 and 0.609, and 10 giving 0.568 and 0.851, the largest, which is the KB Fmax of the railway. A box at the foot carries Formulae 5, 7, 8 and 6, with notes on the 1920 and 960 intervals of the day and the night, the zero for a category at or below 0.1, and the meter to DIN 45669-1, for which a 4 Hz lower limit is usually enough at a railway" width="100%" />

In Example 8 the vertical, measured at the middle of the floor span, is the
direction that decides, and the 2,5 h recorded are taken as representative.
Clause 5 of the draft says where that point goes and what the meter has to
be, and it adds what the 1999 edition did not have: a point at the
foundation that may stand in for the room during a monitoring.

### How the measurement goes

Use a vibration meter to DIN 45669-1; beside a railway a lower frequency
limit of 4 Hz is usually enough. Set up and couple the transducers as
DIN 45669-2 asks, on the floor of the room to be protected, where the
vibration is expected to be strongest: for the vertical, mostly the middle
of the floor span. At a railway, carry the whole measurement out with
particular regard to DIN 45672-1 and DIN 45672-2. Take `z` and the two
horizontals `x` and `y` at right angles, mostly parallel to the outer walls;
the horizontals may also be taken at or near a wall. Record every passage
whole and keep one clock maximum from it, sorted into categories from the
operating programme or timetable, normally at least one per track or
direction and kind of train. Measure long enough to catch the typical
exposure, and mark anything atypical.

For monitoring over weeks or months, a substitute point, at the foundation
for instance, may stand in for the room if it meets DIN 45669-2 and its
transfer to the room, a linear factor or a frequency-dependent function, is
determined, preferably by measurement, with its uncertainty taken only as an
upper bound; from there only `KB_Fmax < A_o` and `KB_FTr < A_r` can be
shown, never reliably an exceedance. Expect up to about 15 % uncertainty on
any `KB_F` quantity, work `KB_Fmax` and `KB_FTr` to at least three decimals
and round them to two as DIN 1333 does, and report the site plan with points
and directions, the coupling, the meter and its settings, and the operating
conditions.
...
[errata page](/phonometry/reference/errata/) has it, with the one condition
...
[1999 edition](/phonometry/vibration/immission/people-in-buildings/); the
...
- [Vibration next to a railway (DIN 45672)](/phonometry/vibration/immission/railway-vibration/):
  ...
  to.
- API reference:
  [`vibration.immission.train_categories`](/phonometry/reference/api/vibration/train-categories/)
  and [`vibration.immission.people`](/phonometry/reference/api/vibration/people/).
