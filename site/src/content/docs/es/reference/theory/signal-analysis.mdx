...
`verify_filter_class` comprueba un banco diseñado contra las máscaras;
...
2. **Diezmado multitasa:** siempre que la mitad de la frecuencia de muestreo
   sea al menos 32 veces el borde superior de la banda, la señal se diezma por
   $M = \lfloor (f_\mathrm{s}/2) / (16\,f_\text{sup}) \rfloor$ antes de filtrar
   (las 18 más graves de las 33 bandas de tercio de octava a 48 kHz), de modo
   que cada banda grave se filtra cerca de su propia frecuencia de muestreo
   natural y sus polos quedan bien dentro del círculo unidad. Se detiene en
   dieciséis veces el borde superior porque, más cerca de su frecuencia de
   Nyquist, la transformación bilineal deforma la banda: a 1,25 veces el borde
   el banco de octavas sumaba sus salidas adyacentes hasta +0,94 dB, más allá
   de los +0,8 dB de la clase 1 en el 5.16 de IEC 61260-1:2014, y a 16 la
   deformación mueve esa suma como mucho 0,005 dB. El nivel de banda se calcula
   sobre la señal diezmada; solo con `sigbands=True` la señal de banda vuelve a
   la frecuencia de entrada, con `resample_poly(M, 1)`. Una banda Chebyshev II
   de orden bajo y banda atenuada profunda, cuyo borde de banda atenuada
   quedaría por encima de dieciséis veces su borde superior, reserva más margen
   para que ese borde quede por debajo del Nyquist diezmado.
