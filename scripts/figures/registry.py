...
from .devices import (
    ...
    generate_enclosure_cabin_insulation,
...
from .vibration import (
    ...
    generate_building_transfer_spectra,
    ...
    generate_machine_count_correction,
    ...
    generate_meter_phase_verification,
    generate_meter_running_rms_decay,
    generate_meter_signal_burst_response,
    generate_meter_tolerance_regions,
    generate_meter_uncertainty_allowance,
    generate_meter_weighting_verification,
    ...
    generate_railway_change_example,
    generate_railway_passage,
    generate_railway_prediction_chain,
...
_FIGURE_FUNCS: tuple[Callable[[str], None], ...] = (
    ...
    generate_cnossos_road_gradient,
    generate_cnossos_road_surfaces,
    # Road surfaces: the ISO 11819-1 statistical pass-by.
    generate_statistical_pass_by,
    generate_pass_by_regression,
    generate_ground_effect_spherical,
    generate_ground_reflection_coefficient,
    ...
    generate_kb_time_response,
    generate_assessment_weighting,
    # Railway vibration (DIN 45672): one passage and its three stretches,
    # the spectra of Figure 6 with Table 1 behind them, and the two printed
    # forms of the compression-wave speed against the continuum.
    generate_railway_passage,
    generate_railway_spectra,
    generate_ground_wave_speeds,
...
def generate_posters(output_dir: str) -> None:
    ...
    slow re-encode of the clips themselves, and it stamps the clips it
    touched the way a render does: the poster extractor is inside the
    fingerprint, so a change to it marks every clip stale, and this is the
    target that brings them back without rendering a frame. It cannot tell a
    poster change from a frame change, so it is run after the former and is
    not a way to silence the check after the latter.
    """
    ...
    clips = sorted({_clip_of(Path(webm).stem) for webm in webms})
    _stamp_clips(clips, output_dir)


def _clip_of(stem: str) -> str:
    """``anim_x`` for ``anim_x``, ``anim_x_dark``, ``anim_x_es`` or ``anim_x_es_dark``."""
    for suffix in ("_es_dark", "_dark", "_es"):
