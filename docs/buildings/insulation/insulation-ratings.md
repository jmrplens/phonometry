...
<picture><source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/jmrplens/phonometry/main/.github/images/diagram_iso717_rating_chain_dark.svg"><img src="https://raw.githubusercontent.com/jmrplens/phonometry/main/.github/images/diagram_iso717_rating_chain.svg" alt="Calculation chain of the ISO 717 single-number ratings. Across the top, a strip of one-third-octave bands from 50 Hz to 5000 Hz: the 16 bands from 100 Hz to 3150 Hz drawn solid with 500 Hz picked out, the five octave bands from 125 Hz to 2000 Hz bracketed under them, and the bands at 50, 63, 80, 4000 and 5000 Hz dashed as an enlarged range that only the adaptation terms read, the 4000 and 5000 Hz bands in ISO 717-1 alone. A box states the rule both parts share: shift the Table 3 curve towards the measurement in 1 dB steps until the sum of unfavourable deviations is as large as possible but not more than 32.0 dB over 16 bands or 10.0 dB over 5, in 0.1 dB steps for the expression of uncertainty, and read the shifted curve at 500 Hz. Below it, two small charts side by side. On the left, ISO 717-1 for airborne sound: the reference curve moved 22 dB down onto a measured sound reduction index, with bars where the measurement falls below it that add up to 31.8 dB, giving Rw = 52 − 22 = 30 dB; C = 28 − 30 = −2 dB from spectrum No. 1, pink noise, and Ctr = 27 − 30 = −3 dB from spectrum No. 2, urban traffic, both over all 16 bands, stated as Rw(C;Ctr) = 30 (−2; −3) dB. On the right, ISO 717-2 for impact sound: the reference curve moved 19 dB up onto a measured normalized impact level, with bars where the measurement rises above it that add up to 28.0 dB, giving Ln,w = 60 + 19 = 79 dB, with 5 dB taken off when the bands are octaves, and CI from the unweighted energy sum of the first 15 bands, without 3150 Hz. Each side lists its enlarged-range terms, and a box at the foot carries Cj = XAj − Xw and CI = Ln,sum − 15 − Ln,w" width="100%"></picture>

*One rule, two engines. Both parts shift their own Table 3 curve the same way,
and the shift differs between them only in the side of the curve a deviation
counts on: below it for airborne insulation, where the curve comes to rest
22 dB down with 31.8 dB of deviations and reads 30 dB at 500 Hz, and above it
for impact sound, where it comes to rest 19 dB up with 28.0 dB and reads
79 dB. Where the bands are octaves, ISO 717-2 takes a further 5 dB off that
reading and ISO 717-1 does not. The adaptation terms then go back to the
measured bands, all 16 of them for $C$ and $C_\mathrm{tr}$ and only the first
15 for $C_\mathrm{I}$.*
...
# ISO 717-2 Annex C worked example: the bare heavy floor of Table C.1,
# laboratory Ln in one-third octaves. CI is read over the 100-2500 Hz sum
# that A.2.1 asks for, not over the sixteen bands the 2020 print of that
# cell sums.
l_n = np.array([62.1, 63.2, 63.5, 66.2, 68.5, 70.0, 71.7, 73.1,
                73.8, 73.5, 73.8, 73.3, 73.1, 73.0, 72.4, 71.2])

# Weighted impact rating + spectrum adaptation term CI (ISO 717-2)
res_imp = building.weighted_impact_rating(l_n)
print(res_imp.rating, res_imp.ci, res_imp.unfavourable_sum)   # 79 -11 28.0  ->  Ln,w(CI)=79(-11)
...
# 16 one-third-octave impact levels Ln, dB (ISO 717-2 Annex C worked
# example, the bare heavy floor of Table C.1; the CI in the title is read
# over the 100-2500 Hz sum that A.2.1 asks for).
l_n = np.array([62.1, 63.2, 63.5, 66.2, 68.5, 70.0, 71.7, 73.1,
                73.8, 73.5, 73.8, 73.3, 73.1, 73.0, 72.4, 71.2])
# Weighted impact rating + spectrum adaptation term CI (ISO 717-2)
res_imp = building.weighted_impact_rating(l_n)
...
# Here the input was Ln, so the rated quantity is Ln,w:
fig, ax = plt.subplots()
...
Feeding this spectrum into `weighted_impact_rating` reproduces the ISO 717-2
Annex C rating (thirds $L_\mathrm{n,w} = 79$; octave 54, $C_\mathrm{I} = 0$)
and, reading $C_\mathrm{I}$ over the 100–2500 Hz sum that A.2.1 specifies,
$C_\mathrm{I} = -11$ for the thirds. The 2020 print of Table C.1 sums all
sixteen bands in that cell and gives $-10$ instead; the registry entry is in
[Errata](../../ERRATA.md).
...
metadata = ReportMetadata(
    ...
    temperature_c=21.5, relative_humidity_percent=45.0, static_pressure_kpa=101.3,
...
| `temperature_c`, `relative_humidity_percent` | `float` | Single representative climate: air temperature (°C, any sign), relative humidity (0–100 %) |
| `static_pressure_kpa` | `float > 0` | Ambient (static) air pressure during the test (kPa) |
| `source_temperature_c`, `source_relative_humidity_percent`, `receiving_temperature_c`, `receiving_relative_humidity_percent` | `float` | Per-room climate when source and receiving rooms are reported separately (same ranges as above) |
