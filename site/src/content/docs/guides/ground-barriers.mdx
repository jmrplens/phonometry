...
`impedance` (a scalar, per-band array, or a `PorousMediumResult`), so a measured
surface impedance in the `e^{-iωt}` convention (a passive ground has
`Im(Z) < 0`, as the porous models return) can be used unchanged.

<ThemeImage src="https://raw.githubusercontent.com/jmrplens/phonometry/main/.github/images/ground_effect_spherical.svg" alt="Excess attenuation (level re free field) against frequency on a log axis for four ground types. Fresh snow (10 kPa) dips deepest and lowest in frequency, near minus 9 dB around 180 Hz; forest floor (50 kPa) and grassland (200 kPa) have progressively shallower, higher-frequency dips; asphalt (20000 kPa) hugs the plus 6 dB hard-ground enhancement limit until a deep dip near 3 kHz. A dotted line marks the plus 6 dB hard-ground limit and a solid line the 0 dB free field" width="90%" loading="eager" />
...
exposes the ground-barrier interference structure a purely energetic sum of
