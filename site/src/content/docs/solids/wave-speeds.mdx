...
Two of the six tables print a longitudinal speed and a loss factor without
saying which of each it is. Long's speed column is headed "Speed of
Longitudinal Waves" and there is no modulus and no Poisson ratio beside it, so
nothing on the page can settle whether it is the bar, the plate or the
unbounded one.
...
Comparing it with the four tables that do say points at the bar speed: 5 050 m/s
is below every plate speed in the catalogue, and Long's aluminium matches Bies'
printed bar speed to the digit. That is a hint and not a reading, and a guess
