...
response to vibration" (clause 1), for hand-transmitted vibration (ISO 5349-1),
whole-body vibration (ISO 2631-1, ISO 2631-2 and ISO 2631-4) and low-frequency
whole-body vibration from 0,1 Hz to 0,5 Hz. **General purpose** separates it
from ISO 8041-2, which specifies personal vibration exposure meters with most
of the same tables (section 11 says which), and not
from some other kind of vibration meter: an instrument for machine condition
monitoring or for damage to buildings is graded against other documents, and
nothing on this page applies to it.

The clause that comes right after those definitions is where this page starts:
how far a real instrument may sit from those weightings before it stops
conforming. That one is a table of frequencies and percentages, and a
subtraction. It turns out not to be alone. The characteristic phase deviation,
the running r.m.s. decay and the saw-tooth signal burst are all specified as
numbers a conforming meter has to produce, and every one of those numbers can
be computed. This page is about the parts of a type test that are arithmetic
rather than laboratory work, and about being clear on where each of them stops.
...
criteria of clause 7 are measurements on hardware, and none of them is here.
What is here is one half of each of the other comparisons: the standard's own
side of them, arithmetic that runs without a bench. The other half always
needs one, because somebody has to shake a transducer and read a display. **Passing the weighting check is a necessary condition for conformity
...
the band is what the verdict is written against and not the closed form. The
verdict keeps that row: `measured_time_s`, `printed_time_s` and `tolerance_s`
beside `passes`, the interval as `lower_time_s` and `upper_time_s`, and
`.plot()` draws the reading on the 10 % rule inside it.
...
single-cycle row of Table 8, nearly three times its 10 % tolerance. Table 6 supplies the
...
## 11. The personal meter of ISO 8041-2

Everything above is ISO 8041-1, the general-purpose meter an engineer carries
to a machine or a seat. ISO 8041-2:2021 specifies the other instrument a
human-vibration survey uses, the **personal vibration exposure meter**
(PVEM): the one left unattended to log a worker's exposure through a full
working day, at least 12 h without anyone touching it (5.1 and 5.12), with its
transducer fitted to the worker, to the seat, or to the machine or power tool
(5.1.2 and 5.1.3). It measures the same quantities through the same
weightings, and Part 2 does not write a second set of tables for it. It prints
the Part 1 tables again wherever it can, so the question for this section is
which of the numbers above a PVEM is held to, and which are its own.

**The same burst, to the digit.** Part 2 5.9 repeats the saw-tooth signal
burst of section 10. Its Table 6 defines the same test signal, and its
Tables 7, 8 and 9 print 228 indications with a tolerance beside each. Read off
the Part 2 pages and set beside `SIGNAL_BURST_RESPONSE`, every one of the 228
indications and every one of the 228 tolerances is the Part 1 number, and
12.13 of Part 2 grades them with the same sentence and the same 3 % maximum
uncertainty. So one implementation serves both parts: the snippets of
section 10 are the Part 2 test as they are written, and the conformance report
grades the library against the Part 2 page in a section of its own, which also
holds every Part 2 cell against its Part 1 twin.

```python
from phonometry import vibration

# The table section 10 reproduces is the Part 2 table too: 72 printed lines
# and 228 cells, the Wk row of Table 8 among them.
...
**Table 2 loses a row.** The tolerances of indication of Part 2 are the first
two rows of the Part 1 table, number for number: 4 % on the indication at the
reference frequency, 5 % for low-frequency whole-body vibration, and 3 %
between a frequency-weighted indication and the band-limited one times the
weighting factor. The third row, the 2 % between the running r.m.s. and the
linear average, is gone, because Part 2 5.13 reads in full "Not applicable for
PVEM". Nor does Part 2 print a decay-time table, so section 9 has no
counterpart in it. The running r.m.s. does not leave the instrument
altogether, though: 5.1.2 still lets a whole-body PVEM report the MTVV, the
largest value its running r.m.s. reaches, and Table 8 grades that MTVV in
the burst exactly as section 10 does.
`PVEM_INDICATION_TOLERANCES_PERCENT` is that table, so a check that walks it
grades a PVEM on what Part 2 prints and on nothing else:
...
# The row Part 2 does not print stays where Part 1 put it.
print(vibration.RUNNING_RMS_CONSISTENCY_TOLERANCE_PERCENT)   # 2.0
```

Two paragraphs of Part 2 12.7 came over from Part 1 unchanged, and neither
fits the meter it now describes. The weighting-consistency test still points
at "the appropriate weighting factor (see Table 1)", and the Part 2 Table 1
still prints 0,388 8 for `Wf`, so the caveat of section 6 applies to a PVEM
word for word: `band_limited_weighting_factor` is the factor that makes the row
satisfiable. And the paragraph after it still grades "each time weighting"
against "the tolerance limits of Table 2", whose time-weighting row Part 2 no
longer prints. Both are in the [errata registry](../../ERRATA.md).

**Its own uncertainties.** Part 2 prints the maximum expanded uncertainty each
of its test clauses allows a laboratory, and in clause 12, pattern evaluation,
every figure is the one Part 1 prints under the same number: 4,5 % for the
mechanical frequency response of 12.11.2, 3 % for the electrical one of
12.11.3, 5 % for the overall response of 12.11.4, 3 % for the signal burst of
12.13. Clause 13 is where the two documents part. Part 2 has no validation of
a one-off instrument, which is clause 13 of Part 1, and its periodic
verification is clause 13 rather than 14: the 13.9 of Part 2 is the 5 % of a
periodic verification, the counterpart of the Part 1 14.9, while the 13.9 of
Part 1 is the 2 % of a different test. A clause number names a different test
in each document, which is why the two tables are kept apart:
...
The decision rule of section 4 does not change: Part 2 12.1 and 13.1 print the
sentence and the coverage factor Part 1 prints, so `verify_weighting` with the
laboratory's own `expanded_uncertainty_percent` is the PVEM verdict too, with
the Part 2 figure as the ceiling on what the laboratory may declare.

What Part 2 adds beyond these tables is not here. The arithmetic of its
12.11, Formulae (9) to (13), splits a frequency-response error between the
transducer and the electrical part of the instrument and combines the
apparent error with the laboratory's uncertainty; the logging test of 12.22
runs the meter for 12 h, with a shaker run at each end, and reads its log file
sample by sample; and logging, operator detection, the detection of transient
artefacts (Annex A) and contact force (Annex B) are properties of the
hardware.

## 12. The same check on the other side of the fence
...
The **personal vibration exposure meter** of ISO 8041-2:2021: its 228 burst
cells of Tables 7 to 9, the Part 1 cells to the digit, graded in the
conformance report against the Part 2 page; its two-row Table 2; and the
maximum expanded uncertainties its clauses 12 and 13 print.
...
**The rest of ISO 8041-2.** The frequency-response arithmetic of its 12.11
(Formulae (9) to (13)), the 12 h logging test of 12.22, and the logging,
operator detection, artefact detection (Annex A) and contact force (Annex B)
...
- International Organization for Standardization. (2017). *Human response to
  ...
  (H.3) and (H.4).
- International Organization for Standardization. (2021). *Human response to
  vibration — Measuring instrumentation — Part 2: Personal vibration exposure
  meters* (ISO 8041-2:2021).
  The personal vibration exposure meter: the 12 h of 5.1 and 5.12, Table 2
  with its two rows and the "Not applicable for PVEM" of 5.13, the saw-tooth
  signal burst of 5.9 with its Table 6 and the printed responses of Tables 7
  to 9, the maximum expanded uncertainties of clauses 12 and 13, and the
  decision rule of 12.1 and 13.1.
...
Part 2: Personal vibration exposure meters*: the saw-tooth signal burst of 5.9
and 12.13, whose Table 6 and Tables 7 to 9 are the Part 1 tables cell for cell
and are served by the same functions; Table 2, with its two rows and without
the running r.m.s. row 5.13 declares not applicable, as
`PVEM_INDICATION_TOLERANCES_PERCENT`; and the maximum expanded uncertainties
of clauses 12 and 13 as `PVEM_MAX_EXPANDED_UNCERTAINTY_PERCENT`, with the
coverage factor of 12.1 and 13.1 as `ISO8041_COVERAGE_FACTOR`.
...
check procedures of the document are not carried out. Of ISO 8041-2:2021, the
frequency-response arithmetic of 12.11 (Formulae (9) to (13)), the 12 h
logging test of 12.22, and the logging, operator detection, artefact detection
(Annex A) and contact force (Annex B) clauses are not implemented.
