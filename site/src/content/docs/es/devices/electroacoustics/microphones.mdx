...
ese margen. La IEC 60268-1 todavía cita la curva por su nombre anterior,
Recomendación 468 del CCIR, y por eso las hojas de características antiguas
imprimen la misma cifra como dB(CCIR); la biblioteca la llama por el documento
vigente y escribe la ponderación `"468"` allí donde toma una. La relación
señal-ruido re 1 Pa (94 dB SPL) se deriva del mismo
...
ponderación ITU-R BS.468-4 está expuesta como `itu_r_468_weighting` en el
...
del **detector de cuasipico** de la ITU-R BS.468-4, que lee un ruido más alto que un
...
ITU-R BS.468-4 (6.2.2) o tercios de octava sin ponderar (6.2.3), porque los tres
...
for b in (0.0, 0.5, 1.0):
    polar = 20 * np.log10(
        np.maximum(np.abs((1 - b) + b * np.cos(np.radians(angles_rad))), 1e-3))
    family = electroacoustics.microphone_characteristics(
        freqs, response, 12.5, tolerance_db=3.0,
        directivity=electroacoustics.MicrophoneDirectivity(polar=(angles_rad, polar),
                                                           frequency=1000.0))
...
vuelve a ponderar nada, así que declarar `weighting="468"` mientras se le pasa
...
ITU-R BS.468-4 ahora sí lo está, en [Medidor de
cuasipico](/phonometry/es/devices/broadcast/quasi-peak/), pero lee un registro y
el apartado 17 toma una tensión o un nivel declarados, así que `weighting="468"`
...
- [Electroacústica](/phonometry/es/devices/electroacoustics/electroacoustics/): el conjunto de
  distorsión de IEC 60268-3, THD+N y SINAD, la THD ponderada ITU-R BS.468-4 y
