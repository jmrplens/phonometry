...
s_t = materials.apparent_dynamic_stiffness(resonant_frequency_hz=intercept,
...
s_gas = np.array([materials.enclosed_gas_stiffness(thickness_m=d * 1e-3,
                                                   porosity=0.9) / 1e6
                  for d in d_mm])
fig, ax = plt.subplots()
...
**kPa·s/m²**. La biblioteca sigue a la norma y lleva la unidad en el nombre
del argumento, `airflow_resistivity_kpa_s_m2`, el único argumento de las
funciones de la EN 29052-1 que no está en SI puro, y solo lo acepta por
nombre, para que la unidad quede escrita en la línea que lo pasa. Una lana mineral medida en 50 000 Pa·s/m² entra
como `50.0`, no como `50000.0`; un
[resultado de ISO 9053](/phonometry/es/materials/absorbers/airflow-resistance/)
se convierte con `sigma / 1000`. Pasar el número en Pa·s/m² caería en la
rama $r \ge 100$, que descarta el término del gas encerrado: para la
...
rama equivocada es una rama válida. Una `ResilientLayer` (más abajo) recibe
esa misma resistividad como `airflow_resistivity_pa_s_m2`, en los Pa·s/m² en
que la guardan todos los catálogos de la biblioteca, y la convierte ella.
...
res = materials.floating_floor_resonance(
    ...
    airflow_resistivity_kpa_s_m2=50.0,   # kPa.s/m2 (= 50000 Pa.s/m2 de la ISO 9053)
    thickness_m=0.020, porosity=0.9,
)
...
### Capas publicadas, cuando no hay valor declarado

El diseño de un suelo flotante parte del $s'$ del fabricante, declarado según
la EN 29052-1 para ese producto. Cuando no lo hay, se publican quince capas
medidas en `PUBLISHED_RESILIENT_LAYERS`, transcritas de la tabla A3 de Hopkins
(p. impresa 610), cuyo pie dice que se midieron según la ISO 9052-1. La
cabecera de la columna imprime $s'$, que la lista de símbolos del propio libro
define como la rigidez dinámica de la capa instalada y separa de la aparente
$s'_\mathrm{t}$, así que todas las filas llevan `dynamic_stiffness_n_m3` y su
frecuencia natural no necesita más que la masa del suelo. Cada capa es una
fila de catálogo como las de cualquier otra tabla publicada: lleva la página
de la que se leyó, y las cuatro espumas rebond llevan la atribución de segundo
nivel que les da la celda impresa.

La clave es `"<tabla>/<fila>"`, como en todos los catálogos: la tabla
empaquetada, `hopkins-2007-table-a3`, y después de qué probeta es la fila,
`<material>_<densidad>_<espesor>`, porque cuatro filas de lana de roca y cuatro
de lana de vidrio solo se diferencian en esos dos números y la tabla impresa
las separa por su posición bajo un nombre que imprime una sola vez.
...
print(round(layer.dynamic_stiffness_n_m3 / 1e6, 1))          # 10.0 MN/m3, como se imprime
print(round(layer.natural_frequency(mass_per_area_kg_m2=120.0), 1))   # 45.9 Hz bajo una losa de 120 kg/m2
...
Son probetas medidas y no valores declarados de producto: sirven para
razonar sobre el orden de magnitud mientras se elige el producto, no como
entrada de un cálculo de cumplimiento. La fila más blanda de la tabla, lana de
vidrio de 40 mm con 7 MN/m³, deja esa misma losa en 38,4 Hz, y la más rígida,
espuma de polietileno de celda cerrada de 5 mm con 115 MN/m³, en 155,8 Hz; ese
margen es toda la cuestión de diseño.

**Una capa que solo da $s'_\mathrm{t}$.** Un informe de ensayo según la
EN 29052-1 da la rigidez aparente $s'_\mathrm{t}$ de su probeta y la rigidez
del gas encerrado $s'_\mathrm{a}$, y solo da $s'$ «si es posible» (apartado
9 e)); una ficha que cita $s'_\mathrm{t}$ sola da menos que el informe del que
sale. En los dos casos $s'_\mathrm{t}$ no es la $s'$ que toma la Fórmula 2: en
una capa permeable al aire el gas encerrado se le suma, y Hopkins señala que a
menudo es un porcentaje significativo de $s'$ (p. 360 impresa). Una
`ResilientLayer` construida a partir de esa fuente la guarda en
`apparent_dynamic_stiffness_n_m3`, y su `natural_frequency` se niega a seguir
hasta que recibe la resistividad al flujo lateral, en Pa·s/m² como la da la
ISO 9053, y, por debajo de 100 kPa·s/m², la rigidez del gas encerrado: la
$s'_\mathrm{a}$ que da el informe, o la Fórmula 7 con el espesor bajo la carga
de ensayo, que el apartado 9 b) también pide al informe y que no es el espesor
nominal que imprime una ficha de producto. Con ellas toma la rama del apartado
8.2 que elige esa resistividad:
...
# d = 30 mm bajo la carga de ensayo, del informe (apartado 9 b)); una s'a
# que da el informe (apartado 9 e)) entra igual, como gas_stiffness_n_m3.
...
f0 = report.natural_frequency(
    ...
    airflow_resistivity_pa_s_m2=50_000.0,   # 50 kPa.s/m2: Fórmula 6, s' = s't + s'a
    gas_stiffness_n_m3=gas,
)
print(round(f0, 1))                                              # 45.3 Hz
```

Leídos como si fueran $s'$, esos mismos 6 MN/m³ habrían dejado la losa en
35,6 Hz, casi 10 Hz por debajo.
...
apartado 9. `ResilientLayer.natural_frequency` aplica la Fórmula 2 a una capa
publicada o construida por el usuario, pasando por el apartado 8.2 cuando la
capa solo da $s'_\mathrm{t}$.
</ScopeClaim>
...
`materials.absorbers.airflow_resistance`, y dan $\sigma$ en Pa·s/m², la
unidad que toma `ResilientLayer.natural_frequency`, mientras que
`airflow_resistivity_kpa_s_m2` va en kPa·s/m². El
