...
[`PlateauMaterial.coincidence_height_db`](/phonometry/reference/api/solids/plateau/#plateaumaterial), which is the depth of the dip
...
| `coincidence_height_db` | How far the transmission loss falls at coincidence, in decibels, measured from the plateau rather than from the mass law. |
| `plateau_frequency_ratio` | The ratio of the two frequencies that bound the plateau, which the page writes B/A. Dimensionless. |
